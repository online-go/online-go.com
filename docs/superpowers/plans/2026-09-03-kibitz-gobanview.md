...
- Streamer mode removed entirely (2026-09-06, user request): the settings toggle, the `is-streamer-mode` styling, the `kibitz.desktop.streamer_mode` session flag, the `kibitz-streamer-mode` body class and the four site-chrome rules that keyed off it (NavBar, Announcements, PrivateChat, toast) are gone, along with the popover's now-unused `isMobileLayout` prop.
