...
const ranked_ranks = (() => {
    ...
    const rankedMin = bounded_rank(
        getUserRating(data.get("user"), "overall", 0).bounded_rank - 9,
    );
    const rankedMax = bounded_rank(
        getUserRating(data.get("user"), "overall", 0).bounded_rank + 9,
    );
...
> {
            ...
            conf: {
                ...
                selected_board_size:
                    standard_board_sizes[
                        `${challenge.game.width}x${challenge.game.height}`
                    ] || "custom",
        ...
        if (this.props.config) {
            if (this.props.config.challenge) {
                state.challenge = Object.assign(
                    ...
                    this.props.config.challenge,
                );
            }

            if (this.props.config.conf) {
                state.conf = Object.assign(
                    this.state.conf,
    ...
    setRanked(tf) {
        ...
        if (tf && this.state.challenge && data.get("user")) {
            next.challenge.game.handicap = Math.min(
                9,
                next.challenge.game.handicap,
            );
    ...
    createChallenge = () => {
        ...
        if (!this.validateBoardSize()) {
            swal(_("Invalid board size, please correct and try again")).catch(
                swal.noop,
            );
            return;
        }
        ...
        */
        ...
        if (
            next.challenge.game.komi_auto === "custom" &&
            next.challenge.game.komi === null
        ) {
            swal(_("Invalid custom komi, please correct and try again")).catch(
                swal.noop,
            );
            return;
        }
        ...
        challenge.game.time_control =
            this.refs.time_control_picker.time_control.system;
        challenge.game.time_control_parameters =
            this.refs.time_control_picker.time_control;
        ...
        */
        ...
        post(
            player_id ? "players/%%/challenge" : "challenges",
            player_id,
            challenge,
        )
            .then((res) => {
                ...
                function active_check() {
                    ...
                    termination_socket.send("game/connect", {
                        game_id: game_id,
                    });
                ...
                function off() {
                    ...
                    termination_socket.send("game/disconnect", {
                        game_id: game_id,
                    });
                    termination_socket.off(
                        `game/${game_id}/gamedata`,
                        onGamedata,
                    );
                    termination_socket.off(
                        `game/${game_id}/rejected`,
                        onRejected,
                    );
    ...
    update_ranked = (ev) =>
        this.setRanked((ev.target as HTMLInputElement).checked);
    ...
    update_challenge_color = (ev) =>
        this.upstate("challenge.challenger_color", ev);
    update_disable_analysis = (ev) =>
                            ...
                            >
                                {bots_list().map((bot, idx) => (
                                    <option key={idx} value={bot.id}>
                                        {bot.username} (
                                        {rankString(getUserRating(bot).rank)})
                                    </option>
                                ))}
                            </select>
                        </div>
    ...
    additionalSettings = () => {
        ...
        const enable_custom_board_sizes =
            mode === "demo" || !this.state.challenge.game.ranked;
                                ...
                                <div className="controls">
                                    <div className="checkbox">
                                        <input
                                            ...
                                            disabled={
                                                this.state.challenge.game
                                                    .private
                                            }
                                            checked={
                                                this.state.challenge.aga_ranked
                                            }
                    ...
                    <div className="form-group">
                        ...
                        <div className="controls">
                            {this.props.playersList ? (
                                <select
                                    value={
                                        this.state.selected_demo_player_black
                                    }
                                    onChange={
                                        this.update_selected_demo_player_black
                                    }
                                >
                                    {this.props.playersList.map(
                                        (player, idx) => (
                    ...
                    <div className="form-group">
                        ...
                        <div className="controls">
                            {this.props.playersList ? (
                                <select
                                    value={
                                        this.state.selected_demo_player_white
                                    }
                                    onChange={
            ...
            >
                            ...
                            >
                                ...
                                <div className="controls">
                                    <div className="checkbox">
                                        <select
                                            value={
                                                this.state.challenge.game.rules
                                            }
                ...
                <div className="right-pane pane form-horizontal">
                    {!this.state.forking_game && (
                        <div
                            className="form-group"
                            id="challenge.game.handicap-group"
                        >
                            ...
                            <div className="controls">
                                <div className="checkbox">
                                    <select
                                        value={
                                            this.state.challenge.game.handicap
                                        }
                            ...
                            <div className="controls">
                                <div className="checkbox">
                                    <input
                                        type="number"
                                        value={
                                            this.state.challenge.game.komi || 0
                                        }
                    ...
                    <div className="form-group">
                        ...
                        <div className="controls">
                            <div className="checkbox">
                                <select
                                    value={
                                        this.state.challenge.challenger_color
                                    }
                    ...
                    <div>
                        ...
                        {(mode === "open" || null) && (
                            <div>
                                <div
                                    className="form-group"
                                    id="challenge-restrict-rank-group"
                                >
                                    <label
                                        className="control-label"
...
export function createBlitz() {
    ...
    return openModal(
        <ChallengeModal config={config} mode={"open"} autoCreate={true} />,
    );
}
...
export function challenge_text_description(challenge) {
    ...
    details_html +=
        ", " +
        interpolate(_("%s handicap"), [
            g.handicap < 0 ? _("auto") : g.handicap,
        ]) +
