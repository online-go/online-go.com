...
export class JosekiAdmin extends React.PureComponent<
    JosekiAdminProps,
    JosekiAdminState
> {
    constructor(props) {
    ...
    revertAllSelectedChanges = () => {
        ...
        this.state.selections.forEach((selected, selection) => {
            if (selected) {
                ...
                reversions.set(
                    selection,
                    `Reversion of audit ${target_id} pending`,
                );
            }
        });
    ...
    reloadData = () => {
        let audits_url =
            ...
            `changes?page=${this.state.current_page + 1}&page_size=${
                this.state.current_pageSize
            }`;
    ...
    showVisitStats = () => {
        openModal(
            <JosekiStatsModal
                fastDismiss
                daily_page_visits={this.state.daily_visits}
            />,
        );
    };
    ...
    render = () => {
        ...
        const audit_type_selections = Object.keys(AuditTypes).map(
            (selection, i) => (
            ...
            <div className="audit-container">
                ...
                <AuditTable
                    ...
                    toggleSelection={(key) => {
                        ...
                        selections.set(
                            key,
                            selections.has(key) ? !selections.get(key) : true,
                        );
                        this.setState({
                            any_selected: Array.from(
                                selections.values(),
                            ).includes(true),
                            selections,
                        });
                    ...
                    columns={[
                        ...
                        },
                        {
                            ...
                            accessor: "user_id",
                            Cell: (props) => (
                                <Player user={props.value}></Player>
                            ),
                        },
                        {
                            Header: _("Date"),
