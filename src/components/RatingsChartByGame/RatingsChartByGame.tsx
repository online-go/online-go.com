...
export class RatingsChartByGame extends React.Component<
...
> {
    ...
    subselect_area = d3
        ...
        .y1((d: RatingEntry) =>
            this.subselect_y(humble_rating(d.rating, d.deviation)),
        );
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (
            this.state.subselect_extents.length !==
            nextState.subselect_extents.length
        ) {
            return true;
        }
        ...
        if (this.state.subselect_extents.length === 2) {
            if (
                this.state.subselect_extents[0] !==
                    nextState.subselect_extents[0] ||
                this.state.subselect_extents[1] !==
                    nextState.subselect_extents[1]
            ) {
                return true;
            }
        }
    ...
    initialize() {
        ...
        this.subselect_chart = this.subselect_graph
            .append("path")
            .attr("class", "area");
    ...
    onSubselectBrush = () => {
        this.subselect_extents =
            (d3.event && d3.event.selection) || this.subselect_x.range();
        ...
        const l = Math.min.apply(
            null,
            this.game_entries.map((d: RatingEntry) =>
                d.index >= this.subselect_extents[0] &&
                d.index <= this.subselect_extents[1]
                    ? Math.min(d.starting_rating, d.rating) - d.deviation
                    : upper,
            ),
        );
    ...
    computeWinLossNumbers() {
        ...
        if (
            this.state.subselect_extents &&
            this.state.subselect_extents.length === 2
        ) {
