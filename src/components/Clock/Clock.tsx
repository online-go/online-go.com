...
export function Clock({
    ...
    const [clock, setClock]: [JGOFClock, (x: JGOFClock) => void] =
        useState(null);
                ...
                {time_control.system === "byoyomi" && (
                    <div className="byo-yomi-container">
                        {(!compact || player_clock.main_time <= 0) && (
                            <React.Fragment>
                                {player_clock.main_time > 0 && (
                                    <span className="periods-delimiter">
                                        {" "}
                                        +{" "}
                                    </span>
                                )}
                        ...
                        >
                            ...
                            {player_clock.periods_left === 1
                                ? pgettext(
                                      "Final byo-yomi period (Sudden Death)",
                                      "SD",
                                  )
...
function prettyTime(ms: number): string {
    ...
    if (ms <= 0 || isNaN(ms)) {
        ...
        ret =
            days === 0
                ? interpolate(
                      pgettext("Game clock: Hours and minutes", "%sh %sm"),
                      [hours, minutes],
                  )
                : interpolate(pgettext("Game clock: hours", "%sh"), [
                      hours + 24,
                  ]);
    } else {
