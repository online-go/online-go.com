...
> {
        ...
        for (const k in GoThemesSorted) {
            ...
            for (const theme of GoThemesSorted[k]) {
                this.canvases[k].push(
                    $("<canvas>")
                        .attr("width", this.state.size)
                        .attr("height", this.state.size),
                );
    ...
    render() {
        const inputStyle = {
            height: `${this.state.size}px`,
            width: `${this.state.size * 1.5}px`,
        };
        const { boardCustom, lineCustom, whiteCustom, blackCustom, urlCustom } =
            this.state;
                ...
                <div className="theme-set">
                    {GoThemesSorted.board.map((theme, idx) => (
                        <div
                            ...
                            className={
                                "selector" +
                                (this.state.board === theme.theme_name
                                    ? " active"
                                    : "")
                            }
                            style={{
                            ...
                            onClick={
                                this.selectTheme["board"][theme.theme_name]
                            }
                        >
                            {theme.theme_name === "Plain" ? (
                                <span>
                                    {pgettext("Custom board theme", "Custom")}
                                </span>
                            ) : (
                                <PersistentElement
                                    elt={this.canvases.board[idx]}
                                />
                            )}
                        </div>
                    ))}
                ...
                <div className="theme-set">
                    {GoThemesSorted.white.map((theme, idx) => (
                        <div
                            ...
                            className={
                                "selector" +
                                (this.state.white === theme.theme_name
                                    ? " active"
                                    : "")
                            }
                            style={theme.styles}
                            onClick={
                                this.selectTheme["white"][theme.theme_name]
                            }
                        >
                ...
                <div className="theme-set">
                    {GoThemesSorted.black.map((theme, idx) => (
                        <div
                            ...
                            className={
                                "selector" +
                                (this.state.black === theme.theme_name
                                    ? " active"
                                    : "")
                            }
                            style={theme.styles}
                            onClick={
                                this.selectTheme["black"][theme.theme_name]
                            }
                        >
    ...
    renderPickers() {
        ...
        for (let i = 0; i < GoThemesSorted.board.length; ++i) {
            ...
            ctx.font =
                "bold " + square_size / 4 + "px Verdana,Courier,Arial,serif";
