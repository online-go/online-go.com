...
interface ChatListProperties {
    ...
    partFunc?: (
        channel: string,
        dont_autoset_active: boolean,
        dont_clear_joined: boolean,
    ) => void;
}
...
> {
    ...
    componentDidMount() {
        if (this.state.collapse_state_store_name) {
            data.watch(
                this.state.collapse_state_store_name,
                this.onCollapseStoreChanged,
            );
        }
    ...
    updateConnectedChannels() {
        let visible_global_channels =
            !this.state.hide_global &&
            this.state.show_unjoined &&
            global_channels.length > 0;
        let visible_group_channels =
            this.state.show_unjoined && group_channels.length > 0;
        ...
        for (let idx = 0; idx < tournament_channels.length; idx = idx + 1) {
            const watched = this._join_or_part(
                "tournament-" + tournament_channels[idx].id,
            );
            if (watched) {
    ...
    toggleShowAllTournamentChannels = () => {
        const collapsed_channel_groups = this.state.collapsed_channel_groups;
        collapsed_channel_groups.tournaments =
            !collapsed_channel_groups.tournaments;
        if (this.state.collapse_state_store_name) {
            data.set(
                this.state.collapse_state_store_name,
                        ...
                        <div className="channel-header">
                            ...
                            <i
                                ...
                                className={
                                    ...
                                    (this.state.collapsed_channel_groups[
                                        "groups"
                                    ]
                                        ? "fa fa-minus"
                                        : "fa fa-plus")
                    ...
                    {group_channels.map((chan) => (
                        ...
                        >
                            <span
                                ...
                                data-channel={"group-" + chan.id}
                            >
                                <img className="icon" src={chan.icon} />{" "}
                                {chan.name}
                            </span>
