...
try {
    ...
    for (const chan of global_channels) {
        try {
            if (
                chan.id === "global-help" &&
                navigator.languages.indexOf("en") >= 0
            ) {
                joined_defaults[chan.id] = true;
            }
            if (
                chan.id === "global-offtopic" &&
                navigator.languages.indexOf("en") >= 0
            ) {
                joined_defaults[chan.id] = true;
            }
...
function autojoin_channels() {
    ...
    for (const chan of global_channels) {
        if (
            chan.id.indexOf("supporter") >= 0 ||
            chan.id.indexOf("shadowban") >= 0
        ) {
...
export function ChatChannelList({
    channel,
}: ChatChannelListProperties): JSX.Element {
    autojoin_channels();

    const joined_channels = data.get("chat.joined");
    const using_resolved_channel = !(
        group_channels.filter((chan) => `group-${chan.id}` === channel).length +
        tournament_channels.filter(
            (chan) => `tournament-${chan.id}` === channel,
        ).length +
        global_channels.filter((chan) => chan.id === channel).length
    );

    const [more, set_more]: [boolean, (tf: boolean) => void] = useState(
        false as boolean,
    );
            ...
            <React.Fragment>
                ...
                <div className="joinable">
                    ...
                    {group_channels
                        .filter(
                            (chan) =>
                                !(`group-${chan.id}` in joined_channels) &&
                                chanSearch(chan),
                        )
                        .map((chan) => (
                            <ChatChannel
                    ...
                    {tournament_channels
                        .filter(
                            (chan) =>
                                !(`tournament-${chan.id}` in joined_channels) &&
                                chanSearch(chan),
                        )
                        .map((chan) => (
                            <ChatChannel
    ...
    return (
        <div className="ChatChannelList">
            ...
            {global_channels
                .filter(
                    (chan) => chan.id in joined_channels || chan.id === channel,
                )
                .map((chan) => (
                    <ChatChannel
                        key={chan.id}
...
export function ChatChannel({
    ...
    const [unread_ct, set_unread_ct]: [number, (x: number) => void] =
        useState(0);
    ...
    if (unread_ct) {
        unread = (
            <span className="unread-count" data-count={`(${unread_ct})`} />
        );
    }
