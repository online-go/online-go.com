...
export function ChatLog(props: ChatLogProperties): JSX.Element {
    ...
    let [showing_games, set_showing_games]: [boolean, (tf: boolean) => void] =
        useState(data.get("chat.show-games", true) as boolean);
    let [height, set_height]: [number, (tf: number) => void] = useState(
    ...
    const game_channel = /^(group-)/.test(props.channel)
        ? props.channel
        : ""; /* global */
...
function ChannelTopic({
    ...
    const [editing, set_editing]: [boolean, (tf: boolean) => void] = useState(
        false as boolean,
    );
    const [topic, set_topic]: [string, (tf: string) => void] = useState("");
    const [topic_updated, set_topic_updated]: [boolean, (tf: boolean) => void] =
        useState(false as boolean);
    const [name, set_name]: [string, (tf: string) => void] = useState(channel);
    const [group_id, set_group_id]: [number | null, (tf: number) => void] =
        useState(null);
    const [tournament_id, set_tournament_id]: [
        number | null,
        (tf: number) => void,
    ] = useState(null);
    ...
    const [title_hover, set_title_hover]: [string, (s: string) => void] =
        useState("");
        ...
        function getTitleHover(topic: TopicMessage | null): string {
            if (topic && topic.username && topic.timestamp) {
                return (
                    topic.username +
                    " - " +
                    moment(new Date(topic.timestamp)).format("LL")
                );
            }
    ...
    return (
        ...
        >
            ...
            <div className="foreground">
                <i
                    className={
                        "header-icon fa fa-list" +
                        (showingChannels ? " active" : "")
                    }
                ...
                {editing && topic_editable ? (
                    <React.Fragment>
                        <i
                            className="header-icon fa fa-save"
                            onClick={saveEdits}
                        />
                    ...
                    <React.Fragment>
                        {topic_editable && (
                            <i
                                className="header-icon fa fa-pencil"
                        ...
                        <div className="channel-topic" title={title_hover}>
                            <div className="topic">
                                <span className="content">
                                    <Linkify>
                                        {localize_time_strings(
                                            profanity_filter(topic.trim()),
                                        ) || name}
                                    </Linkify>
                                </span>
                            </div>
                ...
                <i
                    className={"header-icon fa fa-gear"}
...
function ChatLines({
    ...
    const user = data.get("user");
    const rtl_mode =
        channel in global_channels && !!global_channels[channel].rtl;
            ...
            if (updateTitle) {
                if (document.hasFocus() || !proxy?.channel.unread_ct) {
                    ...
                    window.document.title =
                        `(${proxy?.channel.unread_ct}) ` + _("Chat");
                }
            }
        }
    ...
    const onScroll = useCallback(
        (event: React.UIEvent<HTMLDivElement>): void => {
            ...
            if (tf !== scrolled_to_bottom) {
                ...
                div.className =
                    (rtl_mode ? "rtl chat-lines " : "chat-lines ") +
                    (tf ? "autoscrolling" : "");
            }
            scrolled_to_bottom =
                div.scrollHeight - div.scrollTop - 10 < div.offsetHeight;
        },
        [channel],
    );
...
function ChatInput({
    ...
    const [show_say_hi_placeholder, set_show_say_hi_placeholder] =
        useState(true);
    const [channel_name, set_channel_name] = useState(
        cachedChannelInformation(channel)?.name,
    );
