...
const global_replacements: TextReplacement[] = [
    ...
    },
    {
        ...
        replacement: (m, idx) => (
            <span key={idx}>{m[1] + "tsumegododo" + m[3]}</span>
        ),
    },
    // Match github
    {
        ...
        pattern:
            /\b(https?:\/\/github\.com\/online-go\/online-go\.com\/pull\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <a
                key={idx}
                target="_blank"
    ...
    },
    {
        split: /(%%%PLAYER-[0-9]+%%%)/,
        pattern: /(%%%PLAYER-([0-9]+)%%%)/,
        replacement: (m, idx) => <Player key={idx} user={parseInt(m[2])} />,
    },
    // games
    {
        split: /\b((?:game)[- ]?(?:#)?[0-9]{3,})/i,
        pattern: /(\bgame)[- ]?(?:#)?([0-9]{3,})/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/game/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    {
        ...
        replacement: (m, idx) => (
            <Link
                key={idx}
                to={`/game/${m[3]}${Number(m[4]) >= 0 ? "/" + m[4] : ""}`}
            >
                {(m[2] ? m[2] : "game ") +
                    m[3] +
                    (Number(m[4]) >= 0 ? " move " + m[4] : "")}
            </Link>
        ),
    },
    // reviews
    {
        ...
        replacement: (m, idx) => (
            <Link key={idx} to={`/review/${m[2] || ""}${m[4] || ""}`}>{`${
                m[3] || ""
            }review ${m[2] || ""}${m[4] || ""}`}</Link>
        ),
    },
    {
        split: /\b(review[- ]?(?:#)?[0-9]{3,})/i,
        pattern: /\b(review)[- ]?(?:#)?([0-9]{3,})/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/review/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    {
        ...
        pattern:
            /\b((review )?https?:\/\/online-go\.com\/review(?:\/view)?\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/review/${m[3]}`}>
                {(m[2] ? m[2] : "review ") + m[3]}
            </Link>
        ),
    },
    // demos
    {
        ...
        pattern:
            /\b((demo )?https?:\/\/online-go\.com\/demo(?:\/view)?\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/demo/${m[3]}`}>
                {(m[2] ? m[2] : "demo ") + m[3]}
            </Link>
        ),
    },
    {
        split: /\b(demo[- ]?(?:#)?[0-9]{3,})/i,
        pattern: /\b(demo)[- ]?(?:#)?([0-9]{3,})/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/demo/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    // joseki
    {
        split: /\b(joseki[- ]?(?:#)?[0-9]+)\b/i,
        pattern: /\b(joseki)[- ]?(?:#)?([0-9]+)/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/joseki/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    {
        ...
        pattern:
            /\b((?:joseki )?https?:\/\/online-go\.com\/joseki\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/joseki/${m[2]}`}>
                {"joseki " + m[2]}
            </Link>
    ...
    // groups
    {
        split: /\b(group[ ]?(?:#)?[0-9]+)\b/i,
        pattern: /\b(group)[ ]?(?:#)?([0-9]+)/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/group/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    ...
    },
    {
        ...
        pattern:
            /\b((group )?https?:\/\/online-go\.com\/group\/([0-9]+)(?:\/[^\/<> ]+)*)/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/group/${m[3]}`}>
                {(m[2] ? m[2] : "group ") + m[3]}
            </Link>
        ),
    },
    ...
    },
    {
        ...
        pattern:
            /\b((tournament )?https?:\/\/online-go\.com\/tournaments?\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/tournament/${m[3]}`}>
                {(m[2] ? m[2] : "tournament ") + m[3]}
            </Link>
        ),
    },
    {
        ...
        pattern:
            /\b((tournament |tournament-record )?https?:\/\/online-go\.com\/tournament-records?\/([0-9]+)(?:\/[^\/<> ]+)*(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/tournament-records/${m[3]}`}>
                {(m[2] ? m[2] : "tournament-record ") + m[3]}
            </Link>
        ),
    },
    // ladders
    {
        split: /\b(ladder[- ]?(?:#)?[0-9]+)\b/i,
        pattern: /\b(ladder)[- ]?(?:#)?([0-9]+)/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/ladder/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    {
        ...
        pattern:
            /\b((ladder )?https?:\/\/online-go\.com\/ladder\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/ladder/${m[3]}`}>
                {(m[2] ? m[2] : "ladder") + "-" + m[3]}
            </Link>
        ),
    },
    // puzzles
    {
        split: /\b(puzzle[- ]?(?:#)?[0-9]+)\b/i,
        pattern: /\b(puzzle)[- ]?(?:#)?([0-9]+)/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/puzzle/${m[2]}`}>
                {m[1] + "-" + m[2]}
            </Link>
        ),
    },
    {
        ...
        pattern:
            /\b((puzzle )?https?:\/\/online-go\.com\/puzzle\/([0-9]+)(?:\/|\b))/i,
        replacement: (m, idx) => (
            <Link key={idx} to={`/puzzle/${m[3]}`}>
                {(m[2] ? m[2] : "puzzle ") + m[3]}
            </Link>
        ),
    },
    // learning-hub
    {
        ...
        replacement: (m, idx) => (
            <Link
                key={idx}
                to={`/learn-to-play-go/${m[3]}` + (m[4] ? `/` + m[4] : ``)}
            >
                {(m[2] ? m[2] : "tutorial ") +
                    m[3] +
...
export function chat_markup(
    ...
    if (body.length > lengthLimit) {
        return [
            <span key="message-too-long">&lt;{_("Message too long")}&gt;</span>,
        ];
    }
