...
export class RatingsChart extends React.Component<
    RatingsChartProperties,
    RatingsChartState
> {
    container = null;
    chart_div;
    ...
    rating_line = d3
        ...
        .y((d: RatingEntry) =>
            this.ratings_y(humble_rating(d.rating, d.deviation)),
        );
    ...
    deviation_area = d3
        ...
        .y0((d: RatingEntry) =>
            this.ratings_y(Math.min(d.starting_rating, d.rating) - d.deviation),
        )
        .y1((d: RatingEntry) =>
    ...
    timeline_area = d3
        ...
        .y1((d: RatingEntry) =>
            this.timeline_y(humble_rating(d.rating, d.deviation)),
        );
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        const size_text = nextProps.size
            ? `${nextProps.size}x${nextProps.size}`
            : "";
        this.legend_label.text(
            `${speed_translation(nextProps.speed)} ${size_text}`,
        );
    }
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (
            ...
            nextState.hovered_date !== null
        ) {
            if (
                this.state.hovered_date.getTime() !==
                nextState.hovered_date.getTime()
            ) {
                return true;
            }
            ...
            this.state.hovered_month !== null &&
            nextState.hovered_month !== null
        ) {
            if (
                is_same_month(this.state.hovered_month, nextState.hovered_month)
            ) {
                return true;
            }
            ...
            if (
                this.state.date_extents.length !== nextState.date_extents.length
            ) {
                return true;
            }
            if (this.state.date_extents.length === 2) {
                if (
                    this.state.date_extents[0].getTime() !==
                        nextState.date_extents[0].getTime() ||
                    this.state.date_extents[1].getTime() !==
                        nextState.date_extents[1].getTime()
                ) {
                    return true;
                }
            }
    ...
    initialize() {
        ...
        const boundDataLegendX = (x: number) => {
            return Math.min(
                width - date_legend_width / 2,
                Math.max(date_legend_width / 2, x),
            );
        };
        ...
        this.svg = d3
            ...
            .attr(
                "height",
                height + margin.top + margin.bottom + win_loss_bars_height,
            );
                ...
                this.svg
                    ...
                    .on("mousemove", function () {
                        const x0 = self.ratings_x.invert(
                            ...
                            d3.mouse(this as d3.ContainerElement)[0],
                        );
                        ...
                        self.dateLegend.attr(
                            "transform",
                            "translate(" +
                                (boundDataLegendX(self.ratings_x(midDate)) +
                                    margin.left) +
                                "," +
                                (margin.top +
                                    win_loss_bars_start_y +
                                    win_loss_bars_height +
                                    23) +
                                ")",
                        );
        ...
        this.timeline_graph = this.svg
            ...
            .attr(
                ...
                "translate(" +
                    margin2.left +
                    "," +
                    (margin2.top + win_loss_bars_height) +
                    ")",
            );
        ...
        const size_text = this.props.size
        ...
        this.rating_chart = this.rating_graph
            .append("path")
        ...
        this.y_axis_rank_labels = this.rating_graph
            .append("g")
            .attr("class", "y axis");
        ...
        */
        ...
        this.mouseArea = this.svg
            ...
            .on("mousemove", function () {
                ...
                const d =
                    x0.getTime() - d0.ended.getTime() >
                    d1.ended.getTime() - x0.getTime()
                        ? d1
                        : d0;
                    ...
                    format_date(new Date(d.ended)) +
                        ...
                        interpolate(
                                  ...
                                  ),
                            {
                                ...
                                rating: Math.floor(
                                    humble_rating(d.rating, d.deviation),
                                ),
                ...
                self.dateLegend.attr(
                    "transform",
                    "translate(" +
                        (boundDataLegendX(self.ratings_x(d.ended)) +
                            margin.left) +
                        "," +
        ...
        this.timeline_chart = this.timeline_graph
            .append("path")
            .attr("class", "area");
    ...
    onResize = (no_debounce: boolean = false) => {
        ...
        this.x_axis_date_labels.attr(
            "transform",
            "translate(0 ," + height + ")",
        );
        this.y_axis_rating_labels.attr("transform", "translate(0, 0)");
        this.y_axis_rank_labels.attr(
            "transform",
    ...
    loadDataAndPlot = (data) => {
        ...
        if (this.game_entries.length > 0) {
            ...
            for (const d of this.game_entries) {
                ...
                if (this.games_by_day.length >= 2) {
                    cur_day.increase =
                        this.games_by_day[this.games_by_day.length - 2].rating <
                        cur_day.rating;
                }
                ...
                if (this.games_by_month.length >= 2) {
                    cur_month.increase =
                        this.games_by_month[this.games_by_month.length - 2]
                            .rating < cur_month.rating;
                } else {
        ...
        date_range[0] = new Date(
            date_range[0].getUTCFullYear(),
            date_range[0].getUTCMonth(),
        );
        date_range[1] = new Date(
            date_range[1].getUTCFullYear(),
            date_range[1].getUTCMonth(),
        );
        ...
        this.date_extents = this.timeline_x
            .range()
            .map(this.timeline_x.invert, this.timeline_x);
        this.setState({ date_extents: this.date_extents.slice() });

        this.range_label.text(
            format_date(new Date(date_range[0])) +
                " - " +
                format_date(new Date(date_range[1])),
        );
    ...
    plotWinLossBars = () => {
        ...
        const Y = (count: number) => {
            return (
                win_loss_bars_start_y -
                Math.max(0, win_loss_bars_height - this.win_loss_y(count))
            );
        };
        ...
        this.win_loss_bars.push(
            this.win_loss_graphs[0]
                ...
                .attr(
                    "y",
                    (d: RatingEntry) =>
                        Y(d.count) + H(d.strong_losses + d.strong_wins),
                )
                .attr("width", (d: RatingEntry) =>
                    W(d, d.weak_wins / (d.weak_losses + d.weak_wins || 1)),
                )
                .attr("height", (d: RatingEntry) =>
                    H(d.weak_losses + d.weak_wins),
                ),
        );
        this.win_loss_bars.push(
            ...
            this.win_loss_graphs[2]
                ...
                .attr("width", (d: RatingEntry) =>
                    W(
                        d,
                        d.strong_losses /
                            (d.strong_losses + d.strong_wins || 1),
                    ),
                )
                .attr("height", (d: RatingEntry) =>
                    H(d.strong_losses + d.strong_wins),
                ),
        );
        this.win_loss_bars.push(
            this.win_loss_graphs[3]
                ...
                .attr("width", (d: RatingEntry) =>
                    W(
                        d,
                        d.strong_wins / (d.strong_losses + d.strong_wins || 1),
                    ),
                )
                .attr("height", (d: RatingEntry) =>
                    H(d.strong_losses + d.strong_wins),
        ...
        this.win_loss_bars.push(
            this.win_loss_graphs[4]
                ...
                .attr("x", (d: RatingEntry) => X(d, 0))
                .attr("y", (d: RatingEntry) =>
                    Y(this.max_games_played_in_a_month),
                )
                .attr("width", (d: RatingEntry) => W(d, 0.999))
                .attr("height", (d: RatingEntry) =>
                    H(this.max_games_played_in_a_month - d.count),
                ),
        );
    };
    ...
    onTimelineBrush = () => {
        this.date_extents =
            (d3.event && d3.event.selection) || this.timeline_x.range();
        this.date_extents = this.date_extents.map(
            this.timeline_x.invert,
            this.timeline_x,
        );
    ...
    computeWinLossNumbers() {
        ...
        if (this.state.hovered_date) {
            ...
            if (
                this.state.date_extents &&
                this.state.date_extents.length === 2
            ) {
