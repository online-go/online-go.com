...
export class SeekGraph extends TypedEventEmitter<Events> {
        ...
        for (let i = 0; i < lst.length; ++i) {
            ...
            if ("game_started" in e) {
                ...
                if (e.challenge_id in this.challenges) {
                    const uid =
                        this.challenges[e.challenge_id].system_message_id;
                ...
                if (data.get("user").anonymous) {
                    ...
                    e.min_rank <= this.userRank() &&
                    e.max_rank >= this.userRank()
                ) {
                    e.eligible = true;
                } else {
                    e.eligible = false;
                ...
                try {
                    const move_time = computeAverageMoveTime(
                        e.time_control_parameters,
                    );
                } catch (err) {
    ...
    redraw() {
        ...
        const blitz_line = Math.round(
            SeekGraph.blitz_line_ratio * (w - padding) + padding,
        );
        const live_line = Math.round(
            SeekGraph.live_line_ratio * (w - padding) + padding,
        );
    ...
    drawAxes() {
        ...
        const axis_color = $(document.body).hasClass("light")
            ? "#000000"
            : "#dddddd";
        ...
        try {
            ...
            ctx.fillText(
                word,
                padding + (blitz_line - metrics.width) / 2,
                h - 2,
            );
            ...
            ctx.fillText(
                ...
                padding +
                    blitz_line +
                    (live_line - blitz_line - metrics.width) / 2,
                h - 2,
            );
            ...
            ctx.fillText(
                ...
                padding +
                    live_line +
                    (w - live_line - padding - metrics.width) / 2,
                h - 2,
            );
        } catch (e) {}
        ctx.restore();
    ...
    popupChallengeList(ev) {
        ...
        const list = (this.list = $("<div>").addClass(
        ...
        header.append(
            $("<span>").html(
                rankString({ ranking: first_hit.rank, pro: first_hit.pro }),
            ),
        );
        header.append(
            $("<i>")
        ...
        if (first_hit.time_per_move) {
            ...
            header.append(
                $("<span>").addClass("pull-right").html(_("No time limit")),
            );
        }
        list.append(header);
                    ...
                    $("<i>")
                        ...
                        .click((ev) => {
                            ...
                            del("challenges/%%", C.challenge_id)
                                ...
                                .catch((response) =>
                                    swal(_("Error removing challenge")),
                                );
                        }),
                );
            } else {
            ...
            if (C.live_game) {
                ...
                ReactDOM.render(
                    <Player
                        user={{
                            user_id: 0,
                            ranking: C.black_rank,
                            username: C.black_username,
                        }}
                        rank
                        nolink
                    />,
                ...
                ReactDOM.render(
                    <Player
                        user={{
                            user_id: 0,
                            ranking: C.white_rank,
                            username: C.white_username,
                        }}
                        rank
                        nolink
                    />,
                ...
                if (C.challenger_color !== "automatic") {
                    ...
                    details_html +=
                        ", " +
                        interpolate(pgettext("color", "you play as %s"), [
                            yourcolor,
                        ]);
                }
                ...
                if (C.time_control !== "none") {
                    try {
                        details_html +=
                            ", " +
                            interpolate(
                                pgettext("time control", "%s %s timing"),
                                [
                                    shortShortTimeControl(
                ...
                if (!data.get("user").anonymous) {
                    if (C.min_rank > this.userRank()) {
                        details_html +=
                            ", <span class='cause'>" +
                            interpolate(_("min. rank: %s"), [
                                rankString(C.min_rank),
                            ]) +
                            "</span>";
                    } else if (C.max_rank < this.userRank()) {
                        details_html +=
                            ", <span class='cause'>" +
                            interpolate(_("max. rank: %s"), [
                                rankString(C.max_rank),
                            ]) +
                            "</span>";
                    } else if (
                        C.ranked &&
                        Math.abs(this.userRank() - C.rank) > 9
                    ) {
                        details_html +=
                            ", <span class='cause'>" +
                            _("rank difference more than 9") +
                            "</span>";
                    }
                }
