...
export class GameList extends React.PureComponent<GameListProps, GameListState> {
    ...
    /** True if this list player is on the black side (handles rengo teams). */
    isPlayerBlack(game: GameType, player_id: number | undefined): boolean {
        if (player_id === undefined) {
            return false;
        }
        const clock = game.goban?.last_clock || game.json?.clock;
        if (clock && (clock as AdHocClock).black_player_id === player_id) {
            return true;
        }
        if (clock && (clock as AdHocClock).white_player_id === player_id) {
            return false;
        }
        const teams = game.json?.rengo_teams;
        if (teams?.black?.some((u) => u.id === player_id)) {
            return true;
        }
        if (teams?.white?.some((u) => u.id === player_id)) {
            return false;
        }
        // Fall back to the historic black/white fields on the game object
        return game.black?.id === player_id;
    }
    ...
    extractClockSortFieldsAdHoc(
        ...
        // Use team membership for rengo so we don't mis-pick the clock (#2452)
        const is_black = this.isPlayerBlack(game, this.props.player?.id);
    ...
    extractClockSortFields(game: GameType, use_this_player: boolean) {
        ...
        // Prefer team/color membership over turn color (rengo-safe) (#2452)
        const is_black = this.isPlayerBlack(game, this.props.player?.id);
        const use_black = use_this_player === is_black;
    ...
    applyCurrentSort(games: GameType[]) {
        switch (this.state.sort_order) {
            ...
            case "opponent":
                games.sort((a, b) => {
                    try {
                        const a_on_black = this.isPlayerBlack(a, this.props.player?.id);
                        const b_on_black = this.isPlayerBlack(b, this.props.player?.id);
                        const a_opponent = a_on_black ? a.white : a.black;
                        const b_opponent = b_on_black ? b.white : b.black;
                        return (
