...
> {
    ...
    sortBy(name: SortOrder) {
        return () => {
            if (this.state.sort_order === name) {
                this.setState({
                    sort_order: ("-" + name) as DescendingSortOrder,
                });
            } else {
    ...
    render() {
            ...
            switch (this.state.sort_order) {
                ...
                case "clock":
                    lst.sort((a, b) => {
                        try {
                            const a_clock =
                                a.goban && a.goban.last_clock
                                    ? a.goban.last_clock
                                    : a.json.clock;
                            const b_clock =
                                b.goban && b.goban.last_clock
                                    ? b.goban.last_clock
                                    : b.json.clock;

                            /* not my move? push to bottom (or top) */
                            if (
                                a_clock.current_player ===
                ...
                case "opponent":
                    lst.sort((a, b) => {
                        try {
                            const a_opponent =
                                a.black.id === this.props.player.id
                                    ? a.white
                                    : a.black;
                            const b_opponent =
                                b.black.id === this.props.player.id
                                    ? b.white
                                    : b.black;
                            return (
                                a_opponent.username.localeCompare(
                                    b_opponent.username,
                                ) || a.id - b.id
                            );
                        } catch (e) {
        ...
        if (lst.length === 0) {
            return (
                <div className="container">{this.props.emptyMessage || ""}</div>
            );
        } else if (
            this.props.forceList ||
            lst.length > preferences.get("game-list-threshold")
        ) {
            const sortable =
                this.props.disableSort && this.props.player ? "" : " sortable ";
                    ...
                    {this.props.player ? (
                        <div className="GobanLineSummaryContainerHeader">
                            ...
                            <div
                                ...
                                className={
                                    sortable + game_sort + " text-align-left"
                                }
                            >
                                {_("Game")}
                            </div>
                            ...
                            <div
                                onClick={this.sortBy("clock")}
                                className={sortable + clock_sort}
                            >
                                {_("Clock")}
                            </div>
                            <div
