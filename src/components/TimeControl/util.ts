...
export function timeControlDescription(time_control) {
    ...
    switch (
        time_control &&
        (time_control.system || time_control.time_control)
    ) {
        case "simple":
...
export function shortTimeControl(time_control) {
    ...
    switch (time_control.system || time_control.time_control) {
        case "simple":
            return interpolate(
                pgettext("Simple time: <time>/move", "%s/move"),
                [durationString(time_control.per_move)],
            );
        case "fischer":
...
export function shortDurationString(seconds) {
    ...
    return (
        "" +
        (weeks
            ? " " + interpolate(pgettext("Short time (weeks)", "%swk"), [weeks])
            : "") +
        (days
            ? " " + interpolate(pgettext("Short time (days)", "%sd"), [days])
            : "") +
        (hours
            ? " " + interpolate(pgettext("Short time (hours)", "%sh"), [hours])
            : "") +
        (minutes
            ? " " +
              interpolate(pgettext("Short time (minutes)", "%sm"), [minutes])
            : "") +
        (seconds
            ? " " +
              interpolate(pgettext("Short time (seconds)", "%ss"), [seconds])
            : "")
    );
}
