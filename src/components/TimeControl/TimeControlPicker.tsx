...
> {
    ...
    constructor(props) {
        ...
        const speed =
            data.get("time_control.speed", "correspondence") ||
            "correspondence";
        ...
        this.state = Object.assign(
            this.state,
            makeTimeControlParameters(this.state),
        );
    ...
    UNSAFE_componentWillReceiveProps(next_props: TimeControlPickerProperties) {
        ...
        if (this.props.value !== next_props.value) {
            update = Object.assign(
                update,
                makeTimeControlParameters(next_props.value),
            );
        }
    ...
    syncTimeControl(update: any) {
        ...
        if (this.state.system === "fischer") {
            if (
                findIndex(
                    options["fischer"]["initial_time"],
                    tc.initial_time,
                ) === -1
            ) {
                tc.initial_time = goodChoice(
                    options["fischer"]["initial_time"],
                );
            }
            if (
        ...
        if (this.state.system === "canadian") {
            if (
                findIndex(options["canadian"]["main_time"], tc.main_time) === -1
            ) {
                tc.main_time = goodChoice(options["canadian"]["main_time"]);
            }
            if (
                findIndex(
                    options["canadian"]["period_time"],
                    tc.period_time,
                ) === -1
            ) {
                console.log(
        ...
        if (this.state.system === "byoyomi") {
            if (
                findIndex(options["byoyomi"]["main_time"], tc.main_time) === -1
            ) {
                tc.main_time = goodChoice(options["byoyomi"]["main_time"]);
            }
            if (
                findIndex(options["byoyomi"]["period_time"], tc.period_time) ===
                -1
            ) {
                console.log(
                    "Failed to find",
                    tc.period_time,
                    options["byoyomi"]["period_time"],
                );
        ...
        if (this.state.system === "absolute") {
            if (
                findIndex(options["absolute"]["total_time"], tc.total_time) ===
                -1
            ) {
    ...
    update_speed_bracket = (ev) =>
        this.setSpeedBracket((ev.target as HTMLSelectElement).value);
    update_time_control_system = (ev) =>
        this.setTimeControlSystem((ev.target as HTMLSelectElement).value);
    update_initial_time = (ev) =>
    ...
    update_period_time = (ev) =>
        this.syncTimeControl({ period_time: parseInt(ev.target.value) });
    update_periods = (ev) =>
        this.syncTimeControl({
            periods: Math.max(1, Math.min(300, parseInt(ev.target.value))),
        });
        ...
        return (
            <div className="TimeControlPicker">
                ...
                <div className="form-group">
                    <label
                        className="control-label"
                        htmlFor="challenge-time-control"
                    >
                        {_("Time Control")}
                    </label>
                    <div className="controls">
                ...
                {(this.state.system === "fischer" || null) && (
                    <div
                        id="challenge-max-time-group"
                        className="form-group challenge-time-group"
                    >
                        <label
                            className=" control-label"
                            htmlFor="challenge-max-time"
                        >
                            {_("Max Time")}
                        </label>
                        <div className="controls">
                ...
                {((this.state.speed === "correspondence" &&
                    this.state.system !== "none") ||
                    null) && (
                    <div
                    ...
                    >
                        <label
                            className="control-label"
                            htmlFor="challenge-pause-on-weekends"
                        >
                            {_("Pause on Weekends")}
                        </label>
                        <div className="controls">
