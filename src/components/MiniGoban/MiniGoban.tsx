        ...
        this.goban = new Goban(
            {
                ...
                width:
                    this.props.width ||
                    (this.props.json ? this.props.json.width : 19),
                height:
                    this.props.height ||
                    (this.props.json ? this.props.json.height : 19),
            },
            this.props.json,
        );
    ...
    sync_state() {
        ...
        if (!black) {
            fetch(this.goban.engine.config.black_player_id)
                .then((player) => {
                    this.setState({ black_name: player.username });
                })
                .catch(() => {
        ...
        if (!white) {
            fetch(this.goban.engine.config.white_player_id)
                .then((player) => {
                    this.setState({ white_name: player.username });
                })
                .catch(() => {
                    console.log("Couldn't work out who played white");
                });
        }
        ...
        if (this.props.title) {
            ...
            this.setState({
                ...
                game_date: this.goban.config.end_time
                    ? moment(
                          new Date(this.goban.config.end_time * 1000),
                      ).format("LLL")
                    : "",
        ...
        return (
            <React.Fragment>
                ...
                <div className="inner-container">
                    ...
                    {!this.props.noText && (
                        <div
                            className={`title-black ${this.state.black_to_move_cls}`}
                        >
                            <span className={`player-name`}>
                                {this.state.black_name}
                            </span>
                            <span className={`player-rank`}>
                                {this.state.black_rank}
                            </span>
                            {this.state.finished || (
                                <Clock
                                    compact
                    ...
                    {!this.props.noText && (
                        <div
                            className={`title-white ${this.state.white_to_move_cls}`}
                        >
                            <span className={`player-name`}>
                                {this.state.white_name}
                            </span>
                            <span className={`player-rank`}>
                                {this.state.white_rank}
                            </span>
                            {this.state.finished || (
                                <Clock
                                    compact
