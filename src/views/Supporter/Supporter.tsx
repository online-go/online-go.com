...
export class Supporter extends React.PureComponent<RouteComponentProps, SupporterState> {
    ...
    */
    ...
    getSupportLevel(): number {
        return Math.round(
            this.getAmount() / (getIntervalScale(this.state.interval) * getCurrencyScale(this.state.currency)),
        );
    }
        ...
        const supporter_level = (
            <div className="supporter-text-container">
                ...
                <div id="supporter-ai-perks">
                    <div
                        className={"supporter-perk-box clickable " + (this.getSupportLevel() === 3 ? "active" : "")}
                    ...
                    >
                        ...
                        <div className="price">
                            <span>
                                {formatMoney(
                                    this.state.currency,
                                    3 * getCurrencyScale(this.state.currency) * getIntervalScale(this.state.interval),
                                    true,
                                )}{" "}
                    ...
                    <div
                        className={"supporter-perk-box clickable " + (this.getSupportLevel() === 5 ? "active" : "")}
                    ...
                    >
                        ...
                        <div className="price">
                            <span>
                                {formatMoney(
                                    this.state.currency,
                                    5 * getCurrencyScale(this.state.currency) * getIntervalScale(this.state.interval),
                                    true,
                                )}{" "}
                                / {this.state.interval === "one time" ? _("year") : _(this.state.interval)}
                            </span>
                    ...
                    <div
                        className={"supporter-perk-box clickable " + (this.getSupportLevel() === 10 ? "active" : "")}
                    ...
                    >
                        ...
                        <div className="price">
                            <span>
                                {formatMoney(
                                    this.state.currency,
                                    10 * getCurrencyScale(this.state.currency) * getIntervalScale(this.state.interval),
                                    true,
                                )}{" "}
                                / {this.state.interval === "one time" ? _("year") : _(this.state.interval)}
                            </span>
                    ...
                    <div
                        className={"supporter-perk-box clickable " + (this.getSupportLevel() === 25 ? "active" : "")}
                    ...
                    >
                        ...
                        <div className="price">
                            <span>
                                {formatMoney(
                                    this.state.currency,
                                    25 * getCurrencyScale(this.state.currency) * getIntervalScale(this.state.interval),
                                    true,
                                )}{" "}
                                / {this.state.interval === "one time" ? _("year") : _(this.state.interval)}
                            </span>
            ...
            return (
                <div id="Supporter">
                    ...
                    <h1 style={{ textAlign: "center" }}>
                        <i>
                            Please{" "}
                            <Link to="/sign-in#/user/supporter" className="btn primary">
                                {_("Sign In")}
                            </Link>{" "}
                            to support
                        </i>
                                ...
                                {this.state.amount === 0 ? (
                                    ...
                                    <div className="donation-summary">
                                        ...
                                        <span className="supporter-amount">
                                            {formatMoney(this.state.currency, this.getAmount())}
                                        </span>
                                        / {this.renderIntervalSelect()}
                                    </div>
                                )}
                            ...
                            <div className="supporter-payment-buttons">
                                <div className="stripe">
                                    <button
                                        ...
                                        disabled={this.state.disable_payment_buttons || this.state.processing}
                                    >
                                    ...
                                    >
                                        ...
                                        {this.state.interval !== "one time" && (
                                            <input
                                                type="hidden"
                                                name="t3"
                                                value={this.state.interval === "month" ? "M" : "Y"}
                                            />
                                        )}

                                        {this.state.interval === "one time" && (
                                            <input type="hidden" name="amount" value={this.getAmount().toFixed(2)} />
                                        )}
                                        ...
                                        <input
                                            type="hidden"
                                            name="notify_url"
                                            value={`https://${data.get("config.server_name")}/merchant/paypal_postback`}
                                        />
                                    </form>
                                    <button
                                        className="paypal-button"
                                        disabled={!isPaypalEnabled(this.state.currency)}
                            ...
                            {false && data.get("user").id === 1 && (
                                <button className="danger" onClick={this.DEPRECATEDprocessBraintreeCC}>
                                    {interpolate(`Braintree {{amount}}/month`, {
                                        amount: `$${toFixedWithLocale(this.getAmount(), 2)}`,
                                    })}
                                </button>
                            )}
                        </div>
                                            ...
                                            <React.Fragment>
                                                {recurring_donation.order_id && (
                                                    <button
                                                        ...
                                                        style={{ marginTop: "3em" }}
                                                        onClick={() =>
                                                            this.updateStripePaymentMethod(recurring_donation.order_id)
                                                        }
                                                ...
                                                <button
                                                    className="btn"
                                                    style={{ marginTop: "3em" }}
                                                    onClick={() => this.cancelRecurringDonation(recurring_donation.id)}
                                                    disabled={this.state.processing}
                                                >
                ...
                components={{
                    Option: ({ innerRef, innerProps, isFocused, isSelected, data }) => (
                        <div
                            ...
                            {...innerProps}
                            className={
                                "currency-option " + (isFocused ? "focused " : "") + (isSelected ? "selected" : "")
                            }
                        >
                    ...
                    ValueContainer: ({ children }) => <div className="currency-option-container">{children}</div>,
                }}
            />
        );
    }
