...
const amount_steps = {
    month: [3.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 50.0, 0],
    year: [],
    "one time": [],
};
...
for (let i = 0; i < amount_steps.month.length; ++i) {
    ...
    amount_steps["one time"][i] =
        getIntervalScale("one time") * amount_steps.month[i];
}
...
const locale_details = [{ name: "ja", rtl: 1 }];
...
const active_currency_list = currency_list.filter(
    (currency) => currency.iso !== "ARS" && currency.iso !== "INR",
);
...
function formatMoney(
    currency: string,
    n: number,
    no_fraction_digits: boolean = false,
): string {
...
export class Supporter extends React.PureComponent<
    RouteComponentProps,
    SupporterState
> {
    refs: {
    ...
    componentDidMount() {
        ...
        if (!DEPRECATED_stripe_checkout_js_promise) {
            DEPRECATED_stripe_checkout_js_promise = new Promise<void>(
                (resolve, reject) => {
                    const script = document.createElement("script");
            ...
            stripe_checkout_js_promise = new Promise<void>(
                (resolve, reject) => {
                    const script = document.createElement("script");
                    ...
                    script.charset = "utf-8";
                    script.onload = () => {
                        window["stripe"] = stripe = new Stripe(
                            data.get("config").stripe_pk,
                        );
                        resolve();
                    };
                    script.onerror = () => {
        ...
        if (!data.get("user").anonymous) {
            DEPRECATED_stripe_checkout_js_promise.then(() => {
                get("me/supporter")
                    .then((supporter) => {
                        this.setState(
                            Object.assign({ loading: false }, supporter),
                        );
                    })
                    .catch(errorAlerter);

                get("me/purchase_transactions", {
                    order_by: "-created",
                    page_size: 1,
                })
                    .then((res) => {
                        this.setState({
                            last_transaction: res.results.length
                                ? res.results[0]
                                : null,
                        });
                    })
    ...
    */
    setCurrency = (currency_option) => {
        ...
        const custom_amount_scale =
            (1.0 / getCurrencyScale(this.state.currency)) *
        ...
        if (currency) {
            this.setState({
                ...
                custom_amount: Math.round(
                    this.state.custom_amount * custom_amount_scale,
                ),
            });
    ...
    getStripeAmount() {
        ...
        return (
            this.getAmount() *
            Math.pow(10, getCurrencyDecimals(this.state.currency))
        );
    }
        ...
        promise
            .then((purchase) => {
                ...
                if (purchase.type === "braintree") {
                    ...
                    console.log(
                        "BT payment successful, order id ",
                        purchase.order_id,
                    );
                }
    ...
    getSupportLevel(): number {
        return Math.round(
            this.getAmount() /
                (getIntervalScale(this.state.interval) *
                    getCurrencyScale(this.state.currency)),
        );
    }
    ...
    setSupportLevel(amount: number) {
        ...
        for (; step < amount_steps[this.state.interval].length; ++step) {
            const e = 0.0001;
            if (
                amount > amount_steps[this.state.interval][step] - e &&
                amount < amount_steps[this.state.interval][step] + e
            ) {
                break;
            }
        ...
        const supporter_level = (
            <div className="supporter-text-container">
                ...
                <div id="supporter-ai-perks">
                    <div
                        className={
                            "supporter-perk-box clickable " +
                            (this.getSupportLevel() === 3 ? "active" : "")
                        }
                        onClick={() => this.setSupportLevel(3)}
                    >
                        <div className="title">
                        ...
                        <div className="text">
                            <div>
                                {_(
                                    "AI reviews are already quite strong, reads a few moves deep",
                                )}
                            </div>
                        </div>
                        <div className="price">
                    ...
                    <div
                        className={
                            "supporter-perk-box clickable " +
                            (this.getSupportLevel() === 5 ? "active" : "")
                        }
                        onClick={() => this.setSupportLevel(5)}
                    >
                        <div className="title">
                        ...
                        <div className="text">
                            <div>
                                {_(
                                    "Moderately deep reading in your AI reviews",
                                )}
                            </div>
                        </div>
                        <div className="price">
                    ...
                    <div
                        className={
                            "supporter-perk-box clickable " +
                            (this.getSupportLevel() === 10 ? "active" : "")
                        }
                        onClick={() => this.setSupportLevel(10)}
                    >
                        <div className="title">
                        ...
                        <div className="text">
                            <div>
                                {_(
                                    "Deep reading, very good analysis in most situations",
                                )}
                            </div>
                        </div>
                        <div className="price">
                    ...
                    <div
                        className={
                            "supporter-perk-box clickable " +
                            (this.getSupportLevel() === 25 ? "active" : "")
                        }
                        onClick={() => this.setSupportLevel(25)}
                    >
                        <div className="title">
                            <span>
                                {pgettext(
                        ...
                        <div className="text">
                            <div>
                                {_(
                                    "Very deep reading, intended for the most serious students",
                                )}
                            </div>
                        </div>
                        <div className="price">
        ...
        if (user.anonymous) {
            return (
                <div id="Supporter">
                    <h2>
                        <span>
                            <i className="fa fa-star"></i> {_("Support OGS")}
                        </span>
                    </h2>
            ...
            <div id="Supporter">
                <h2>
                    ...
                    {this.state.recurring_donations.length > 0 && (
                        <span> {_("Thank you for your support!")}</span>
                    )}
                </h2>
                    ...
                    <div id="supporter-payment-block">
                        ...
                        */}
                                ...
                                <div className="stripe">
                                    ...
                                    <div className="powered-by-stripe">
                                        <a href="https://stripe.com/">
                                            <img
                                                src={`${cdn_release}/img/powered_by_stripe.svg`}
                                            />
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div className="paypal">
                                    ...
                                    >
                                        ...
                                        <input
                                            type="hidden"
                                            name="item_name"
                                            value="Supporter Account"
                                        />
                                        {this.state.interval !== "one time" && (
                                            <input
                                                type="hidden"
                                                name="a3"
                                                value={this.getAmount().toFixed(
                                                    2,
                                                )}
                                            />
                                        )}
                                        ...
                                        <input
                                            type="hidden"
                                            name="custom"
                                            value={data.get("user").id}
                                        />
                                        <input
                                            id="paypal-purchase-id"
                                            type="hidden"
                                            name="invoice"
                                            value=""
                                        />
                                    ...
                                    <button
                                        ...
                                        dir={getDirection(current_language)}
                                        onClick={
                                            isPaypalEnabled(this.state.currency)
                                                ? this.processPaypal
                                                : null
                                        }
                                    >
                                        <img
                                            src={`${cdn_release}/img/new_paypal.png`}
                                        />
                                    </button>
                                </div>
                            </div>
                            ...
                            {false && data.get("user").id === 1 && (
                                <button
                                    className="danger"
                                    onClick={this.DEPRECATEDprocessBraintreeCC}
                                >
                                    {interpolate(`Braintree {{amount}}/month`, {
                                        amount: `$${toFixedWithLocale(
                                            this.getAmount(),
                                            2,
                                        )}`,
                                    })}
                                        ...
                                        <div className="recurring-donation">
                                            <p>
                                                {interpolate(
                                                    interval === "month"
                                                        ? _(
                                                              "You are currently supporting us with {{amount}} per month from your {{card_type}} card ending in {{last_four}} and expiring on {{month}}/{{year}}, thanks!",
                                                          )
                                                        : interval === "year"
                                                        ? _(
                                                              "You are currently supporting us with {{amount}} per year from your {{card_type}} card ending in {{last_four}} and expiring on {{month}}/{{year}}, thanks!",
                                                          )
                                                ...
                                                <React.Fragment>
                                                    {recurring_donation.order_id && (
                                                        <button
                                                            ...
                                                            onClick={() =>
                                                                this.updateStripePaymentMethod(
                                                                    recurring_donation.order_id,
                                                                )
                                                            }
                                                            disabled={
                                        ...
                                        <div className="recurring-donation">
                                            <p>
                                                {interpolate(
                                                    interval === "month"
                                                        ? _(
                                                              "You are currently supporting us with {{amount}} per month from your paypal account, thanks!",
                                                          )
                                                        : interval === "year"
                                                        ? _(
                                                              "You are currently supporting us with {{amount}} per year from your paypal account, thanks!",
                                                          )
                                                        : "<ERROR: amount = {{amount}} interval = {{interval}}>",
                                                    {
                                                        amount: formatMoney(
            ...
            <Select
                ...
                components={{
                    Option: ({
                        ...
                        >
                            <span className="iso">{data.iso}</span>
