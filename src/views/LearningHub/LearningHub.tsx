...
export class LearningHub extends React.PureComponent<LearningHubProperties> {
    ...
    _render() {
        const section_name = (
            this.props.match.params.section || "index"
        ).toLowerCase();
        let section;
                    ...
                    {sections.map((arr) => (
                        <div key={arr[0]} className="section">
                            ...
                            <div className="contents">
                                {arr[1].map((S) => {
                                    const className = getSectionClassName(
                                        S.section(),
                                    );
                        ...
                        <div className="contents">
                            ...
                            <CardLink className={"done"} to={`/puzzles`}>
                                ...
                                <div>
                                    <h1>
                                        {pgettext(
                                            "Practice go by playing puzzles",
                                            "Puzzles",
                                        )}
                                    </h1>
                                    <h3>
                                        {_("Practice by solving Go puzzles")}
                                    </h3>
                                </div>
                            </CardLink>
                        </div>
        ...
        return (
            <div className="LearningHub-section-nav">
                <Link to="/learn-to-play-go/">
                    ...
                    {pgettext("Learning hub menu", "Menu")}
                </Link>
                ...
                {sections.map((arr) => (
                    <div key={arr[0]} className="section">
                        ...
                        {arr[1].reduce(
                            (acc, v) =>
                                acc + (v.section() === section_name ? 1 : 0),
                            0,
                        ) // is our active section?
                            ? arr[1].map((S) => {
                                  return (
                                      <Link
                                          key={S.section()}
