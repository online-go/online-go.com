...
export abstract class LearningPage extends React.Component<LearningPageProperties> {
    ...
    next = () => {
        ...
        if (this.props.curpage + 1 < this.props.npages) {
            browserHistory.push(
                window.location.pathname.replace(/\/[0-9]+/, "") +
                    "/" +
    ...
    render() {
        ...
        const correct: boolean =
            this.correct_answer_triggered || this.complete();
        const fail: boolean =
            this.error_triggered ||
            this.wrong_answer_triggered ||
            this.failed();
    ...
    at(coord: string): number {
        if (this.instructional_goban && this.instructional_goban.goban) {
            const obj = this.instructional_goban.goban.engine.decodeMoves(
                coord,
                9,
                9,
            );
            return this.instructional_goban.goban.engine.board[obj[0].y][
                obj[0].x
            ];
        }
        return 0;
