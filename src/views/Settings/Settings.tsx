...
ITC.register("logout", (device_uuid) => {
    if (device_uuid !== data.get("device.uuid", "")) {
        swal("This device has been logged out remotely")
            .then(logout)
            .catch(logout);
    }
});
...
function logoutOtherDevices() {
    swal({
        ...
        .then((password) => {
            ...
            swal("Other devices have been logged out")
                .then(ignore)
                .catch(ignore);
        })
        .catch(ignore);
...
export function Settings({
    ...
    const [settings_state, setSettingsState]: [
        SettingsState,
        (s: SettingsState) => void,
    ] = React.useState({});
    const [vacation_base_time, set_vacation_base_time]: [
        number,
        (s: number) => void,
    ] = React.useState(Date.now());
    const [loaded, set_loaded]: [number, (b: number) => void] =
        React.useState(0);
            ...
            .then((settings) => {
                if (!canceled) {
                    ...
                    setSettingsState({
                        ...
                        vacation_left: durationString(
                            settings.profile.vacation_left,
                        ),
                        hide_ui_class: settings.site_preferences.hide_ui_class,
                        self_reported_account_linkages:
                            settings.self_reported_account_linkages,
                    });
                    set_loaded(1);
                }
            })
    ...
    let SelectedPage: (props: SettingGroupProps) => JSX.Element = () => (
        <div>Error</div>
    );
    ...
    return (
        <div className="PreferenceLine">
            <span className="PreferenceLineTitle">
                {props.title}
                {props.description && (
                    <div className="PreferenceLineDescription">
                        {props.description}
                    </div>
                )}
            </span>
...
function SettingsGroupSelector(props: {
    children: React.ReactNode;
}): JSX.Element {
    ...
    return (
        <div>
            ...
            <div className="vacation-container">
                ...
                <div>
                    {interpolate(
                        _("You have {{vacation_left}} of vacation available"),
...
function AccountSettings(props: SettingGroupProps): JSX.Element {
    const [password1, setPassword1]: [string, (x: string) => void] =
        React.useState("");
    const [password2, setPassword2]: [string, (x: string) => void] =
        React.useState("");
    const [email, __setEmail]: [string, (x: string) => void] = React.useState(
        props.state.profile.email,
    );
    const [email_changed, setEmailChanged]: [boolean, (x: boolean) => void] =
        React.useState();
    const [message, setMessage]: [string, (x: string) => void] =
        React.useState("");
            ...
            swal({
                ...
                .then((password) => {
                    post("/api/v0/changePassword", {
                        ...
                        .then((obj) => {
                            swal(_("Password updated successfully!")).catch(
                                swal.noop,
                            );
                        })
                        .catch(errorAlerter);
...
function ChatPreferences(props: SettingGroupProps): JSX.Element {
    ...
    const [group_chat_unread, _setGroupChatUnread]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("chat-subscribe-group-chat-unread"));
    const [group_chat_mentions, _setGroupChatMentions]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("chat-subscribe-group-mentions"));
    const [tournament_chat_unread, _setTournamentChatUnread]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(
        preferences.get("chat-subscribe-tournament-chat-unread"),
    );
    const [tournament_chat_mentions, _setTournamentChatMentions]: [
        boolean,
        (x: boolean) => void,
    ...
    return (
        <div>
            ...
            >
                <Toggle
                    checked={group_chat_mentions}
                    onChange={toggleGroupChatMentions}
                />
            </PreferenceLine>
            ...
            >
                <Toggle
                    checked={group_chat_unread}
                    onChange={toggleGroupChatUnread}
                />
            </PreferenceLine>
            ...
            >
                <Toggle
                    checked={tournament_chat_unread}
                    onChange={toggleTournamentChatUnread}
                />
            </PreferenceLine>
        </div>
    );
                ...
                {blocked_players.map((block_state) => {
                    ...
                    return (
                        <div key={user_id} className="blocked-player-row">
                            <span className="blocked-player">
                                {block_state.username}
                            </span>
                            <BlockPlayerModal
                                playerId={user_id}
                                inline={true}
...
function AnnouncementPreferences(props: SettingGroupProps): JSX.Element {
    ...
    const [mute_stream_announcements, _muteStreamAnnouncements]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("mute-stream-announcements"));
    const [mute_event_announcements, _muteEventAnnouncements]: [
        boolean,
        (x: boolean) => void,
                ...
                columns={[
                    {
                        ...
                        render: (a) =>
                            moment(a.timestamp).format("YYYY-MM-DD LTS"),
                    },
                    {
                        header: "Duration",
                        className: "",
                        render: (a) => {
                            const ms = moment(a.expiration).diff(
                                moment(a.timestamp),
                            );
...
function GamePreferences(props: SettingGroupProps): JSX.Element {
    const [dock_delay, _setDockDelay]: [number, (x: number) => void] =
        React.useState(preferences.get("dock-delay"));
    const [ai_review_enabled, _setAiReviewEnabled]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("ai-review-enabled"));
    const [variations_in_chat, _setVariationsInChat]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("variations-in-chat-enabled"));
    const [live_submit_mode, _setLiveSubmitMode]: [
        string,
        (x: string) => void,
    ] = React.useState(getSubmitMode("live"));
    const [corr_submit_mode, _setCorrSubmitMode]: [
        string,
        (x: string) => void,
    ] = React.useState(getSubmitMode("correspondence"));
    const [board_labeling, _setBoardLabeling]: [string, (x: string) => void] =
        React.useState(preferences.get("board-labeling"));

    const [autoadvance, _setAutoAdvance]: [boolean, (x: boolean) => void] =
        React.useState(preferences.get("auto-advance-after-submit"));
    const [always_disable_analysis, _setAlwaysDisableAnalysis]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("always-disable-analysis"));
    const [dynamic_title, _setDynamicTitle]: [boolean, (x: boolean) => void] =
        React.useState(preferences.get("dynamic-title"));
    const [function_keys_enabled, _setFunctionKeysEnabled]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("function-keys-enabled"));
    const [autoplay_delay, _setAutoplayDelay]: [number, (x: number) => void] =
        React.useState(preferences.get("autoplay-delay") / 1000);
    ...
    function getSubmitMode(speed) {
        const single = preferences.get(
            `one-click-submit-${speed}` as ValidPreference,
        );
        const dbl = preferences.get(
            `double-click-submit-${speed}` as ValidPreference,
        );
    ...
    return (
        <div>
                ...
                <span>
                    &nbsp;
                    {
                        dock_delay === MAX_DOCK_DELAY
                            ...
                            : interpolate(_("{{number_of}} seconds"), {
                                  number_of: dock_delay,
                              }) // translators: Indicates the number of seconds to delay the slide out of the panel of game buttons on the right side of the game page
                    }
                </span>
            </PreferenceLine>
            ...
            <PreferenceLine
                title={_("Auto-advance to next game after making a move")}
            >
            ...
            >
                <Toggle
                    checked={always_disable_analysis}
                    onChange={setAlwaysDisableAnalysis}
                />
            </PreferenceLine>
            ...
            <PreferenceLine
                title={_("Enable function keys for game analysis shortcuts")}
            >
                <Toggle
                    checked={function_keys_enabled}
                    onChange={setFunctionKeysEnabled}
                />
            </PreferenceLine>
            ...
            >
                <Toggle
                    checked={!variations_in_chat}
                    onChange={toggleVariationsInChat}
                />
            </PreferenceLine>
...
function GeneralPreferences(props: SettingGroupProps): JSX.Element {
    const [profanity_filter, _setProfanityFilter]: [
        Array<string>,
        (x: Array<string>) => void,
    ] = React.useState(Object.keys(preferences.get("profanity-filter")));
    const [game_list_threshold, _setGameListThreshold]: [
        number,
        (x: number) => void,
    ] = React.useState(preferences.get("game-list-threshold"));
    const [desktop_notifications, _setDesktopNotifications]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("desktop-notifications"));
    const [show_offline_friends, _setShowOfflineFriends]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("show-offline-friends"));
    const [unicode_filter_usernames, _setUnicodeFilterUsernames]: [
        boolean,
        (x: boolean) => void,
    ...
    const [rating_graph_always_use, _setAlwaysUse]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("rating-graph-always-use"));
    const [rating_graph_plot_by_games, _setUseGames]: [
        boolean,
        (x: boolean) => void,
    ...
    const [hide_incident_reports, setHideIncidentReports]: [
        boolean,
        (x: boolean) => void,
    ] = React.useState(preferences.get("hide-incident-reports"));

    const user = data.get("user");
    const desktop_notifications_enableable: boolean =
    ...
    return (
        <div>
            ...
            <PreferenceLine title={_("Profanity filter")}>
                <Select
                    ...
                    options={language_options.filter(
                        (lang) => lang.value in profanity_regex,
                    )}
            ...
            <PreferenceLine title={_("Show offline friends on list")}>
                <Toggle
                    checked={show_offline_friends}
                    onChange={setShowOfflineFriends}
                />
            </PreferenceLine>

            <PreferenceLine
                title={_("Hide special unicode symbols in usernames")}
            >
                <Toggle
                    checked={unicode_filter_usernames}
                    onChange={setUnicodeFilterUsernames}
                />
            </PreferenceLine>
            ...
            <PreferenceLine title={_("Show tournament indicator")}>
                <Toggle
                    checked={show_tournament_indicator}
                    onChange={setShowTournamentIndicator}
                />
            </PreferenceLine>
            ...
            <PreferenceLine title={_("Plot rating graph by")}>
                <span>{_("Ask me")}</span>
                <Toggle
                    checked={rating_graph_always_use}
                    onChange={setAlwaysUse}
                />
    ...
    function set(key: string): (value: any) => void {
        return (value: any) => {
            ...
            update_link_preferences_debounce = setTimeout(() => {
                ...
                put("me/settings", {
                    self_reported_account_linkages:
                        props.state.self_reported_account_linkages,
                })
    ...
    return (
        <div id="LinkPreferences">
            ...
            >
                <input
                    ...
                    placeholder={pgettext(
                        "Go association Identifier or PIN number",
                        "ID or PIN",
                    )}
                    value={link.org1_id || ""}
                ...
                />
                <RankSelect
                    value={link.org1_rank}
                    onChange={set("org1_rank")}
                />
            </PreferenceLine>
            ...
            >
                ...
                />
                <RankSelect
                    value={link.org2_rank}
                    onChange={set("org2_rank")}
                />
            </PreferenceLine>
            ...
            >
                ...
                />
                <RankSelect
                    value={link.org3_rank}
                    onChange={set("org3_rank")}
                />
            </PreferenceLine>
            ...
            <PreferenceLine title={_("Little Golem")}>
                ...
                />
                <RankSelect
                    value={link.golem_rank}
                    onChange={set("golem_rank")}
                />
            </PreferenceLine>
            <PreferenceLine title={_("WBaduk")}>
                ...
                />
                <RankSelect
                    value={link.wbaduk_rank}
                    onChange={set("wbaduk_rank")}
                />
            </PreferenceLine>
            <PreferenceLine title={_("Tygem")}>
                ...
                />
                <RankSelect
                    value={link.tygem_rank}
                    onChange={set("tygem_rank")}
                />
            </PreferenceLine>
            ...
            <PreferenceLine title={_("Yike Weiqi")}>
                ...
                />
                <RankSelect
                    value={link.yike_rank}
                    onChange={set("yike_rank")}
                />
            </PreferenceLine>
            <PreferenceLine title={_("GoQuest")}>
                ...
                />
                <RankSelect
                    value={link.goquest_rank}
                    onChange={set("goquest_rank")}
                />
            </PreferenceLine>
        </div>
    );
...
function AssociationSelect({
    ...
    try {
        if (data.get("user").country) {
            ...
            if (
                associations.filter(
                    (a) => a.country === data.get("user").country,
                ).length > 0
            ) {
...
function SoundPreferences(props: SettingGroupProps): JSX.Element {
    const [tick_tock_start, __setTickTockStart]: [number, (x: number) => void] =
        React.useState(preferences.get("sound.countdown.tick-tock.start"));
    const [ten_seconds_start, __setTenSecondsStart]: [
        number,
        (x: number) => void,
    ] = React.useState(preferences.get("sound.countdown.ten-seconds.start"));
    const [five_seconds_start, __setFiveSecondsStart]: [
        number,
        (x: number) => void,
    ] = React.useState(preferences.get("sound.countdown.five-seconds.start"));
    const [every_second_start, __setEverySecondStart]: [
        number,
        (x: number) => void,
    ] = React.useState(preferences.get("sound.countdown.every-second.start"));
    const [count_direction, __setCountDirection]: [
        string,
        (x: string) => void,
    ] = React.useState(preferences.get("sound.countdown.byoyomi-direction"));
    ...
    const start_options = [
        {
            value: 0,
            label: pgettext("Never play the countdown sound", "Never"),
        },
        {
            value: 60,
            label: pgettext(
                "Start playing the countdown sound at 60 seconds",
                "60 seconds",
            ),
        },
        {
            value: 45,
            label: pgettext(
                "Start playing the countdown sound at 45 seconds",
                "45 seconds",
            ),
        },
        {
            value: 30,
            label: pgettext(
                "Start playing the countdown sound at 30 seconds",
                "30 seconds",
            ),
        },
        {
            value: 20,
            label: pgettext(
                "Start playing the countdown sound at 20 seconds",
                "20 seconds",
            ),
        },
        {
            value: 15,
            label: pgettext(
                "Start playing the countdown sound at 15 seconds",
                "15 seconds",
            ),
        },
        {
            value: 10,
            label: pgettext(
                "Start playing the countdown sound at 10 seconds",
                "10 seconds",
            ),
        },
        {
            value: 5,
            label: pgettext(
                "Start playing the countdown sound at 5 seconds",
                "5 seconds",
            ),
        },
        {
            value: 3,
            label: pgettext(
                "Start playing the countdown sound at 3 seconds",
                "3 seconds",
            ),
        },
    ];
    ...
    const start_options_tens = [
        ...
        },
        {
            value: 50,
            label: pgettext(
                "Start playing the countdown sound at 50 seconds",
                "50 seconds",
            ),
        },
        {
            value: 40,
            label: pgettext(
                "Start playing the countdown sound at 40 seconds",
                "40 seconds",
            ),
        },
        {
    ...
    const counting_direction_options = [
        ...
        {
            value: "down",
            label: pgettext("Announce seconds left counting down", "Down"),
        },
        {
            value: "up",
            label: pgettext("Announce seconds left counting up", "Up"),
        },
    ];
    ...
    return (
        <div>
            <div className="Settings-Card">
                ...
                <div>
                    ...
                    <span>
                        <SoundPackSelect
                            group="game_voice"
                            options={SpriteGroups.game_voice}
                        />
                    </span>
                    <span>
                        <Volume
                            group="game_voice"
                            sample={["byoyomi", "you_have_won"]}
                        />
                    </span>
                </div>
                ...
                <div>
                    ...
                    <span>
                        <SoundPackSelect
                            group="countdown"
                            options={SpriteGroups.countdown}
                        />
                    </span>
                    <span>
                        <Volume
                ...
                <div>
                    ...
                    <span>
                        <Select
                            ...
                            value={
                                start_options.filter(
                                    (opt) => opt.value === every_second_start,
                                )[0]
                            }
                            getOptionValue={(data) => data.value}
                ...
                <div>
                    ...
                    <span>
                        <SoundPackSelect
                            group="stones"
                            options={SpriteGroups.stones}
                        />
                    </span>
                    <span>
                        <Volume
                            group="stones"
                            sample={["black-1", "white-2", "black-3"]}
                        />
                    </span>
                </div>
                ...
                <div>
                    <h4>
                        {pgettext(
                            "Sound pack to use for various effects",
                            "Effects",
                        )}
                    </h4>
                    <span>
                        <SoundPackSelect
                            group="effects"
                            options={SpriteGroups.effects}
                        />
                    </span>
                    <span>
                        <Volume
                            group="effects"
                            sample={["tutorial-bling", "tutorial-pass"]}
                        />
                    </span>
                </div>
            </div>
            ...
            <LineText>
                {pgettext(
                    "Settings for individual sound options",
                ...
                <label className="SoundToggle-voice-label">
                    {voice ? (
                        <span className="voice-or-effect">
                            {pgettext(
                                "Use the spoken voice sound for this sound effect",
                                "Voice",
                            )}
                        </span>
                    ) : (
...
function EmailNotificationToggle(props: {
    ...
    function save(on: boolean): void {
        ...
        up[props.notification] = {
            description:
                props.state.notifications[props.notification].description,
            value: {
                email: on,
                mobile: on,
            },
...
function SoundPackSelect(props: {
    ...
    const [pack_id, __setPackId]: [string, (x: string) => void] =
        React.useState(sfx.getPackId(props.group));
