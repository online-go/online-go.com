...
export class AnnouncementCenter extends React.PureComponent<
    {},
    AnnouncementCenterState
> {
    constructor(props) {
        super(props);
        const exp = new Date();
    ...
    setExpiration = (moment_date) => {
        ...
        const announcement_duration =
            moment_date.toDate().getTime() - Date.now();
        if (
            announcement_duration > MAX_ANNOUNCEMENT_DURATION &&
    ...
    create = () => {
        const announcement_duration =
            moment(this.state.expiration).toDate().getTime() - Date.now();
        if (
            announcement_duration > MAX_ANNOUNCEMENT_DURATION &&
            !data.get("user").is_superuser
        ) {
    ...
    render() {
        ...
        let can_create: string | boolean =
            this.state.expiration && this.state.text;
        const announcement_duration =
            moment(this.state.expiration).toDate().getTime() - Date.now();
        if (
            announcement_duration > MAX_ANNOUNCEMENT_DURATION &&
