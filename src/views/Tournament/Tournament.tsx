...
export class Tournament extends React.PureComponent<
...
> {
    ...
    elimination_tree_container = $(
        `<div class="tournament-elimination-container">`,
    );
    ...
    constructor(props) {
        ...
        const tournament_id =
            parseInt(this.props.match.params.tournament_id) || 0;

        this.state = {
            new_tournament_group_id:
                parseInt(this.props.match.params.group_id) || 0,
        ...
        if (false) {
            ...
            this.state.tournament.time_start = moment(new Date())
                .add(1, "minute")
                .format();
    ...
    UNSAFE_componentWillReceiveProps(next_props) {
        if (
            next_props.match.params.tournament_id !==
            this.props.match.params.tournament_id
        ) {
            this.setState({
                tournament_id: parseInt(next_props.match.params.tournament_id),
            });
    ...
    abort_requests() {
        ...
        abort_requests_in_flight(
            `tournaments/${this.state.tournament_id}/rounds`,
        );
        abort_requests_in_flight(
            `tournaments/${this.state.tournament_id}/players/all`,
        );
    }
    ...
    refreshPlayerList = (tournament_id?) => {
        ...
        ret.then((players) => {
            ...
            if (this.state.rounds.length) {
                new_state.rounds = this.state.rounds.map((r) =>
                    this.groupify(r, players),
                );
            }
        ...
        post("tournaments/%%/players", this.state.tournament_id, {
            ...
            .catch((res) => {
                try {
                    ...
                    this.setState({
                        invite_result: _(JSON.parse(res.responseText).error),
                    });
                } catch (e) {
                    console.error(res);
                    console.error(e);
        ...
        if (
            ...
            const bindHovers = (div, id) => {
                if (typeof id !== "number") {
                    ...
                    console.error(
                        "Tournament bind hover called with non numeric id",
                    );
                }

                div.mouseover(() => {
                    $(".elimination-player-hover").removeClass(
                        "elimination-player-hover",
                    );
                    $(".elimination-player-" + id).addClass(
                        "elimination-player-hover",
                    );
                });
                div.mouseout(() => {
            ...
            for (let round_num = 0; round_num < rounds.length; ++round_num) {
                ...
                for (
                    let match_num = 0;
                    match_num < round.matches.length;
                    ++match_num
                ) {
                    ...
                    black.prepend(
                        $(
                            "<a class='elimination-game'><i class='ogs-goban'></i> </a>",
                        ).attr("href", "/game/view/" + match.gameid),
                    );
                    white.prepend(
                        $(
                            "<a class='elimination-game'><i class='ogs-goban'></i> </a>",
                    ...
                    const obj = {
                        div: matchdiv,
                        black_src:
                            round_num > 0 ? lastbucket[match.black] : null,
                        white_src:
                            round_num > 0 ? lastbucket[match.white] : null,
                        black_won: result === "B",
                        match: match,
            ...
            const playerWon = (obj, player_id) => {
                ...
                if (
                    obj.match.result[0] === "B" &&
                    obj.match.black === player_id
                ) {
                    return true;
                }
                if (
                    obj.match.result[0] === "W" &&
                    obj.match.white === player_id
                ) {
                    return true;
                }
                return false;
            ...
            for (let i = 0; i < all_objects.length; ++i) {
                ...
                if (obj.bye_src) {
                    obj.second_bracket =
                        obj.bye_src.second_bracket ||
                        !playerWon(obj.bye_src, obj.player_id);
                }
            ...
            const layout = (collection) => {
                ...
                arr.sort((a, b) => {
                    ...
                    const compute_rank = (e) => {
                        ...
                        if (
                            ...
                            return (
                                players[e.match.black].ranking +
                                players[e.match.white].ranking
                            );
                        }
                        return -1000;
                    };
                ...
                for (let i = 0; i < all_objects.length; ++i) {
                    if (
                        !all_objects[i].second_bracket &&
                        max_se_round === all_objects[i].round
                    ) {
                        if (
                            all_objects[i].black_src &&
                            all_objects[i].black_src.second_bracket &&
                    ...
                    if (!obj.second_bracket) {
                        if (obj.bye_src) {
                            ...
                            if (
                                ...
                                obj.top =
                                    (obj.black_src.top + obj.white_src.top) /
                                    2.0;
                            } else if (
                                obj.black_src &&
                                obj.black_src.second_bracket ===
            ...
            if (not_laid_out) {
                swal(
                    "Warning: " + not_laid_out + " matches not laid out",
                ).catch(swal.noop);
            }
            ...
            const drawLines = (obj) => {
                        ...
                        drawLine([
                            {
                                ...
                                y: getWinnerBottom(obj.black_src),
                            },
                            {
                        ...
                        drawLine([
                            {
                                ...
                                y: getWinnerBottom(obj.white_src),
                            },
                            {
                ...
                if (obj.bye_src) {
                    ...
                    if (
                        !obj.second_bracket ||
                        obj.second_bracket === obj.bye_src.second_bracket
                    ) {
                        drawLine([
                            {
                                x: obj.bye_src.left,
                                y: getWinnerBottom(obj.bye_src),
                            },
                            {
                                x: obj.bye_src.right,
                                y: getWinnerBottom(obj.bye_src),
                            ...
                            {
                                x: obj.left,
                                y: Math.round((obj.top + obj.bottom) / 2.0),
                            },
                        ]);
                    }
                }
    ...
    groupify(round, players) {
        try {
            ...
            for (let i = 0; i < round.matches.length; ++i) {
                ...
                matches.push({
                    player: players[m.black],
                    opponent: players[m.white],
                });
                matches.push({
                    player: players[m.white],
                    opponent: players[m.black],
                });
                ...
                for (const opponent_id in match_map[player_id].matches) {
                    ...
                    if (ogr && ogr !== group) {
                        ...
                        for (const id in match_map) {
                            if (
                                match_map[id].group === ogr ||
                                match_map[id].group === group
                            ) {
            ...
            let max_len = 0;
            for (let i = 0; i < groups.length; ++i) {
                groups[i].players = groups[i].players.sort(
                    this.compareUserRank,
                );
            ...
            for (let i = 0; i < broken_list.length; ++i) {
                broken_players[broken_list[i].player.id] =
                    broken_list[i].player;
            }
            for (let i = 0; i < broken_list.length; ++i) {
                const opponents = [];
    ...
    save = () => {
        ...
        if (tournament.name.length < 5) {
            ...
            swal(_("Please provide a name for the tournament")).catch(
                swal.noop,
            );
            return;
        }
        ...
        if (tournament.description.length < 5) {
            ...
            swal(_("Please provide a description for the tournament")).catch(
                swal.noop,
            );
            return;
        }
        ...
        if (max_players < 2) {
            this.refs.max_players.focus();
            swal(_("You need at least two players in a tournament")).catch(
                swal.noop,
            );
            return;
        }

        tournament.time_start = moment(new Date(tournament.time_start))
            .utc()
            .format();
        tournament.group =
            this.state.new_tournament_group_id ||
            (this.state.group && this.state.group.id);
        if (!tournament.group) {
            delete tournament.group;
        }
        tournament.time_control_parameters.time_control =
            tournament.time_control_parameters.system;
    ...
    */
    setTournamentType = (ev) => {
        ...
        this.setState({
            tournament: Object.assign({}, this.state.tournament, update),
        });
    };
    setLowerBar = (ev) => {
    ...
    setHandicap = (ev) =>
        this.setState({
            tournament: Object.assign({}, this.state.tournament, {
                handicap: ev.target.value,
            }),
        });
    setBoardSize = (ev) =>
        this.setState({
            tournament: Object.assign({}, this.state.tournament, {
                board_size: ev.target.value,
            }),
        });
    ...
    setMinRank = (ev) =>
        this.setState({
            tournament: Object.assign({}, this.state.tournament, {
                min_ranking: ev.target.value,
            }),
        });
    setMaxRank = (ev) =>
        this.setState({
            tournament: Object.assign({}, this.state.tournament, {
                max_ranking: ev.target.value,
            }),
        });
    ...
    updateNotes = (data) => {
        const newSettings = Object.assign(
            {},
            this.state.tournament.settings,
            data,
        );
        this.setState({
            tournament: Object.assign({}, this.state.tournament, {
    ...
    render() {
        try {
            ...
            if (tournament.time_start) {
                tournament_time_start_text = moment(
            ...
            try {
                maximum_players = parseInt(
                    tournament.settings.maximum_players as string,
                );
            } catch (e) {
                console.error(e);
            }
            ...
            switch (tournament.exclusivity) {
                case "open":
                    tournament_exclusivity = pgettext(
                        "Open tournament",
                        "Open",
                    );
                    break;
                    ...
                    <div className="top-details">
                        <div>
                            ...
                            {editing &&
                                tournament.tournament_type === "opengotha" && (
                                    <h3>
                                        Please note, the OpenGotha tournament is
                                        a manually managed tournament. Please
                                        read the{" "}
                                        <a
                                        ...
                                        <i>
                                            This is a new tournament type,
                                            please report any issues experience.
                                        </i>
                                    </h3>
                                )}
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {!editing ? (
                                                        <span>
                                                            {
                                                                first_pairing_method_text
                                                            }
                                                        </span>
                                                    ) : (
                                                        <select
                                                            value={
                                                                tournament.first_pairing_method
                                                            }
                                                            onChange={
                                                                this
                                                                    .setFirstPairingMethod
                                                            }
                                                        >
                                                            <option
                                                                disabled={
                                        ...
                                        <td>
                                            {!editing ? (
                                                ...
                                                <input
                                                    ...
                                                    id="analysis"
                                                    checked={
                                                        tournament.analysis_enabled
                                                    }
                                                    onChange={
                                                        this.setAnalysisEnabled
                                                    }
                                                />
                                            )}
                                        </td>
                                        ...
                                        <td>
                                            {!editing ? (
                                                ...
                                                <input
                                                    ...
                                                    id="provisional"
                                                    checked={
                                                        !tournament.exclude_provisional
                                                    }
                                                    onChange={
                                                        this
                                                            .setExcludeProvisionalPlayers
                                                    }
                                                />
                                            )}
                                        </td>
                            ...
                            <div className="player-list">
                                {(tournament.exclusivity !== "invite" ||
                                    ...
                                    <div className="invite-input">
                                        ...
                                        <div className="bold">
                                            {this.state.invite_result}
                                        </div>
                                        <div id="tournament-invite-result"></div>
                                    </div>
                                )}
                                {(this.state.sorted_players.length > 0 ||
                                    null) && (
                                    <Card>
                                        {this.state.sorted_players.map(
                                            (player, idx) => (
                                                <div key={player.id}>
                                                    <Player
                                                        icon
                                    ...
                                    <div>
                                        <div className="roster-rounds-line">
                                            ...
                                            <Steps
                                                ...
                                                selected={
                                                    this.state
                                                        .selected_round as number
                                                }
                                                onChange={this.setSelectedRound}
                                            />
                                        </div>
                                        {this.state.selected_round ===
                                        "roster" ? (
                                            <OpenGothaRoster
                                                tournament={tournament}
                                                players={
                                                    this.state.sorted_players
                                                }
                                            />
                                        ) : this.state.selected_round ===
                                          "standings" ? (
                                ...
                                <div className="results">
                                    {this.state.use_elimination_trees ? (
                                        <PersistentElement
                                            elt={
                                                this
                                                    .elimination_tree_container[0] as HTMLDivElement
                                            }
                                        />
                                    ) : (
                                        <div>
                                                        ...
                                                        {raw_selected_round.matches.map(
                                                            (m, idx) => (
                                                                <MiniGoban
                                                                    ...
                                                                    width={
                                                                        tournament.board_size as number
                                                                    }
                                                                    height={
                                                                        tournament.board_size as number
                                                                    }
    ...
    adjustPoints(player_id: number) {
        ...
        swal({
            ...
            text: interpolate(
                pgettext(
                    "How may tournament points to adjust a user by",
                    "Adjustment for %s",
                ),
                [user.username],
            ),
    ...
    disqualify(player_id: number) {
        ...
        swal({
            text: interpolate(_("Really disqualify {{user}}?"), {
                user: user.username,
            }),
            showCancelButton: true,
            focusCancel: true,
    ...
    renderExtraPlayerActions = (player_id: number, ignored: any) => {
        ...
        if (
            !(
                ...
                (this.state.tournament.director &&
                    this.state.tournament.director.id === user.id)
            )
        ) {
            return null;
        }
            ...
            return (
                <div className="actions">
                    <button
                        ...
                        onClick={() => this.adjustPoints(player_id)}
                    >
                        {_("Adjust Points")}
                    </button>
                    <button
                        className="reject xs"
                        onClick={() => this.disqualify(player_id)}
                    >
                        {_("Disqualify")}
                    </button>
                </div>
...
function OpenGothaTournamentRound({
    ...
    const [notes, _set_notes]: [string, (s) => void] =
        React.useState(roundNotes);
    const [notes_updated, set_notes_updated]: [boolean, (b) => void] =
        React.useState(false);
    window["rounds"] = rounds;
    const round_started = !!(
        rounds.length >= selectedRound &&
        (rounds[selectedRound - 1]?.matches.length || 0) > 0
    );
    ...
    function save_notes() {
        ...
        const up: any = {};
        put(`tournaments/${tournament.id}/rounds/${selectedRound}`, {
            notes: notes,
        })
    ...
    function startRound() {
        ...
        swal({
            ...
            .then(() => {
                post(
                    `tournaments/${tournament.id}/rounds/${selectedRound}/start`,
                )
                    .then(ignore)
                    .catch(errorAlerter);
            })
    ...
    if (round_started) {
                        ...
                        <tbody>
                            ...
                            {selected_round.matches
                                ...
                                .map((m, idx) => {
                                    ...
                                    try {
                                        const match =
                                            selected_round.match_map[
                                                m.player.id
                                            ].matches[m.opponent.id];

                                        black =
                                            match.black === m.player.id
                                                ? m.player
                                                : m.opponent;
                                        white =
                                            match.black === m.player.id
        ...
        const roundMatches =
            tournament.settings?.["opengotha-staged-games"]?.[selectedRound] ||
            {};
                ...
                {(tournament.can_administer || null) && (
                    <div className="round-td-controls">
                        <button className="primary" onClick={startRound}>
                            {pgettext(
                                "Start a round of games in a tournament",
                                "Start round",
                            )}
                        </button>
                    </div>
...
function OpenGothaTournamentUploadDownload({
    ...
    function uploadFile(files) {
        put("tournaments/%%/opengotha", tournament.id, files[0])
            ...
            .catch((res) => {
                console.error(res);
                try {
                    openMergeReportModal(
                        res.responseJSON.merge_report,
    ...
    return (
        <Card>
            <h3>
                {pgettext(
                    "Area to upload and download OpenGotha files to",
                    "OpenGotha File Area",
                )}
                <a
...
export function rankRestrictionText(min_ranking, max_ranking) {
    if (min_ranking <= 0) {
        if (max_ranking >= 36) {
            ...
            return interpolate(
                pgettext(
                    "ranks restriction: '<rank> and below'",
                    "%s and below",
                ),
        ...
        if (max_ranking >= 36) {
            return interpolate(
                pgettext(
                    "ranks restriction: '<rank> and above'",
                    "%s and above",
                ),
            ...
            return interpolate(
                pgettext("ranks restriction: '<rank> - <rank>'", "%s - %s"),
                [longRankString(min_ranking), longRankString(max_ranking)],
            );
        }
    }
}
export function shortRankRestrictionText(min_ranking, max_ranking) {
    if (min_ranking <= 0) {
        if (max_ranking >= 36) {
            ...
            return interpolate(
                pgettext("ranks restriction: '<rank> and below'", "%s"),
        ...
        if (max_ranking >= 36) {
            return interpolate(
                pgettext("ranks restriction: '<rank> and above'", "%s+"),
                [rankString(min_ranking)],
            );
        } else {
            return interpolate(
                pgettext("ranks restriction: '<rank> - <rank>'", "%s-%s"),
                [rankString(min_ranking), rankString(max_ranking)],
            );
        }
    }
}
