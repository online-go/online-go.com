    ...
    render() {
        return (
            <div className="Moderator">
                <UIPush
                    event="modlog-updated"
                    channel="moderators"
                    action={this.refreshModlog}
                />
                <UIPush
                    event="new-user"
                    channel="moderators"
                    action={this.refreshUserlog}
                />
...
export class ColorTableToggle extends React.Component<{}, any> {
    ...
    colorTable() {
        ...
        for (let i = 0; i < users.length; i++) {
            ...
            if (m > 1) {
                users[i].getElementsByTagName("td")[3].style.backgroundColor =
                    "hsl(0,80%,20%)";
            }
            const l =
                parseInt(
            ...
            if (
                ...
                users[i].getElementsByTagName("td")[2].style.backgroundColor =
                    "hsl(0,80%,20%)";
            }
            ...
            if (
                ...
                if (k.length === 2) {
                    ...
                    if (greylist.includes(k[1])) {
                        users[i].getElementsByTagName(
                            "td",
                        )[8].style.backgroundColor = "salmon";
                    }
                    const k1 = k[1].split(".");
                    if (
                        k1.length > 1 &&
                        greylist2.includes(k1[k1.length - 1])
                    ) {
                        users[i].getElementsByTagName(
                            "td",
                        )[8].style.backgroundColor = "salmon";
            ...
            if (!users2[i].classList.contains("checked")) {
                ...
                for (let j = 0; j < chars.length; j++) {
                    if (chars[j] >= "A" && chars[j] <= "Z") {
                        s +=
                            '<span style="color:#99ddff">' +
                            chars[j] +
                            "</span>";
                    } else if (chars[j] >= "a" && chars[j] <= "z") {
                        s +=
                            '<span style="color:#99cc77">' +
                            chars[j] +
                            "</span>";
...
export function shadowban(player_id) {
    if (player_id < 0) {
        ...
        return moderate(player_id, "Reason for shadow banning?", {
            is_shadowbanned: 1,
        });
    }
}
export function remove_ban(player_id) {
    if (player_id < 0) {
        ...
        return moderate(player_id, "Reason for removing ban?", {
            is_banned: 0,
        });
    }
}
export function remove_shadowban(player_id) {
    if (player_id < 0) {
        ...
        return moderate(player_id, "Reason for removing the shadow ban?", {
            is_shadowbanned: 0,
        });
    }
}
