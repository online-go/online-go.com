...
export class AIReviewChart extends React.Component<AIReviewChartProperties> {
    ...
    max_score?: number;
    min_score?: number;
    win_rate_area_container?: d3.Selection<
        SVGPathElement,
        unknown,
        null,
        undefined
    >;
    variation_win_rate_line_container?: d3.Selection<
        SVGPathElement,
        unknown,
        null,
        undefined
    ...
    variation_move_crosshair?: d3.Selection<
        SVGLineElement,
        unknown,
        null,
        undefined
    ...
    highlighted_move_circle_container: d3.Selection<
        SVGElement,
        unknown,
        null,
        undefined
    ...
    shouldComponentUpdate(nextProps: AIReviewChartProperties, nextState: any) {
        return (
            ...
            !deepCompare(
                nextProps.variation_entries,
                this.props.variation_entries,
            ) ||
            this.props.updatecount !== nextProps.updatecount ||
            this.props.move_number !== nextProps.move_number ||
            this.props.variation_move_number !==
                nextProps.variation_move_number ||
    ...
    initialize() {
        ...
        this.highlighted_move_circle_container =
            this.prediction_graph.append("g");
        ...
        this.mouse_rect
            ...
            .on("mousemove", function () {
                ...
                self.full_crosshair?.attr(
                    ...
                    "translate(0, " +
                        self.y(
                            self.props.use_score ? d.score : d.win_rate * 100.0,
                        ) +
                        ")",
                );
    ...
    plot() {
        ...
        const use_score_safe =
            this.props.use_score && this.props.ai_review.scores != null;
        ...
        if (this.props.entries.length > 0) {
            ...
            for (let i = 0; i < n_moves_to_render; ++i) {
                const unitNoiseLine =
                    simplex.getValue(Date.now() * 0.001, i * sine_step, 0.5) *
                    0.4;
                entries.push({
        ...
        if (
            ...
            variation_entries.push({
                ...
                score: 0.0,
                move_number:
                    variation_entries[variation_entries.length - 1].move_number,
                num_variations: 0,
            });
        }
        ...
        if (use_score_safe) {
            ...
            this.y.domain(
                d3.extent([this.min_score, this.max_score]) as [number, number],
            );
        } else {
            this.y.domain(d3.extent([0.0, 100.0]) as [number, number]);
        }
        ...
        this.win_rate_area = d3
            ...
            .y1((d) =>
                this.y(use_score_safe ? d.score * 1.0 : d.win_rate * 100.0),
            )
            ...
            .data(
                ...
                data.get("theme") === "light"
                    ? [
                          ...
                          {
                              offset:
                                  (gradient_transition_point - 1).toFixed(0) +
                                  "%",
                              color: "#444444",
                          },
                          {
                              offset:
                                  gradient_transition_point.toFixed(0) + "%",
                              color: "#888888",
                          },
                          {
                              offset:
        ...
        this.highlighted_move_circles
            ...
            .attr("cy", (d) =>
                this.y(use_score_safe ? d.score : d.win_rate * 100),
            )
            .attr("r", (d) => 3)
            .attr("fill", (d) => "#FF0000");
        ...
        try {
            ...
            if (
                (removes as any)._groups[0].length !==
                (adds as any)._groups[0].length
            ) {
    ...
    onResize = (no_debounce: boolean = false) => {
        ...
        this.width = Math.max(
            100,
            (this.container?.clientWidth || 0) - MARGIN.left - MARGIN.right,
        );
