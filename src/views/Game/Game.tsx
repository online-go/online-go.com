...
export class Game extends React.PureComponent<GameProperties, GameState> {
    ...
    constructor(props) {
        ...
        this.conditional_move_tree = $(
            "<div class='conditional-move-tree-container'/>",
        )[0];
        ...
        this.set_analyze_tool = {
            ...
            stone_alternate: this.setAnalyzeTool.bind(
                this,
                "stone",
                "alternate",
            ),
        ...
        this.enterConditionalMovePlanner =
            this.enterConditionalMovePlanner.bind(this);
        ...
        this.goban_setModeDeferredPlay =
            this.goban_setModeDeferredPlay.bind(this);
        ...
        this.goban_jumpToLastOfficialMove =
            this.goban_jumpToLastOfficialMove.bind(this);
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        if (
            this.props.match.params.game_id !==
                nextProps.match.params.game_id ||
            this.props.match.params.review_id !==
                nextProps.match.params.review_id
        ) {
            this.deinitialize();
    ...
    componentDidUpdate(prevProps, prevState) {
        if (
            this.props.match.params.game_id !==
                prevProps.match.params.game_id ||
            this.props.match.params.review_id !==
                prevProps.match.params.review_id
        ) {
            this.initialize();
            this.sync_state();
        }
    ...
    initialize() {
        ...
        const opts: GobanCanvasConfig = {
            ...
            draw_right_labels:
                label_position === "all" ||
                label_position.indexOf("right") >= 0,
            draw_bottom_labels:
                label_position === "all" ||
                label_position.indexOf("bottom") >= 0,
            display_width: Math.min(
                this.ref_goban_container.offsetWidth,
                this.ref_goban_container.offsetHeight,
            ),
            visual_undo_request_indicator: preferences.get(
        ...
        if (this.review_id) {
            ...
            opts.isPlayerOwner = () =>
                this.goban.review_owner_id === data.get("user").id;
            opts.isPlayerController = () =>
                this.goban.review_controller_id === data.get("user").id;
        }
            ...
            this.goban.on("state_text", (state) => {
                ...
                if (state.show_moves_made_count) {
                    ...
                    if (document.hasFocus()) {
                        this.last_move_viewed =
                            this.goban.engine.getMoveNumber();
                        window.document.title = state.title;
                    } else {
                        const diff =
                            this.goban.engine.getMoveNumber() -
                            this.last_move_viewed;
                        window.document.title = interpolate(
                            _("(%s) moves made"),
                            [diff],
                        );
                    }
        ...
        this.goban.on("clock", (clock: JGOFClock) => {
            ...
            if (seconds > 0 && seconds < Math.max(10, every_second_start)) {
                ...
                const count_direction_computed =
                    count_direction !== "auto"
                        ? count_direction
                        : count_direction_auto;

                if (count_direction_computed === "up") {
                    if (seconds < every_second_start) {
                        this.goban.setByoYomiLabel(
                            (every_second_start - seconds).toString(),
                        );
                    }
                } else {
        ...
        */
        ...
        this.goban.on("auto-resign", (data) => {
            if (
                this.goban.engine &&
                data.player_id === this.goban.engine.players.black.id
            ) {
                this.setState({
            ...
            if (
                this.goban.engine &&
                data.player_id === this.goban.engine.players.white.id
            ) {
                this.setState({
        ...
        if (this.review_id) {
            ...
            this.goban.on("review.load-end", () => {
                ...
                if (
                    ...
                    const cur_move_string =
                        this.goban.engine.cur_move.getMoveStringToThisPoint();

                    const prev_last_review_message =
                        this.goban.getLastReviewMessage();
                     ...
                     * authenticated */
                    setTimeout(() => {
                        this.goban.setLastReviewMessage(
                            prev_last_review_message,
                        );
            ...
            get("games/%%", this.game_id)
                .then((game) => {
                    ...
                    if (
                        ...
                        this.on_refocus_title =
                            this.black_username + " vs " + this.white_username;
                        window.document.title = this.on_refocus_title;
                    }
            ...
            get("reviews/%%", this.review_id)
                .then((review) => {
                    if (review.game) {
                        this.setState({
                            historical_black:
                                review.game.historical_ratings.black,
                            historical_white:
                                review.game.historical_ratings.white,
                        });
                    }
                })
                .catch(ignore);
    ...
    private bindAudioEvents(): void {
...

        {
            ...
            this.goban.on(
                ...
                (who: { player_id: number }) => {
                    ...
                    debounce = setTimeout(() => {
                        if (
                            this.goban.engine.playerColor(user?.id) ===
                            "invalid"
                        ) {
        ...
        this.goban.on("audio-clock", (audio_clock_event: AudioClockEvent) => {
            ...
            const tick_tock_start = preferences.get(
            ...
            if (audio_clock_event.in_overtime && !overtime_announced) {
                ...
                if (sfx.hasSoundSample("start_counting")) {
                    ...
                    if (time_control.system === "byoyomi") {
                        ...
                        last_period_announced =
                            audio_clock_event.clock.periods_left;
                    } else {
                    ...
                    if (seconds_left > 60) {
                        ...
                        if (seconds_left < every_second_start) {
                            ...
                            if (
                                ...
                                tick_tock_start > 0 &&
                                seconds_left <= tick_tock_start
                            ) {
                                audio_to_play =
                                    seconds_left % 2 ? "tick" : "tock";
                            } else {
                                audio_to_play = undefined;
                            }
            ...
            if (this.goban.mode === "analyze") {
                ...
                if (
                    ...
                    this.autoplay_timer = setTimeout(
                        step,
                        preferences.get("autoplay-delay"),
                    );
                }
            } else {
                this.stopAutoplay();
    ...
    recenterGoban() {
        ...
        $(this.goban_div).css({
            top:
                Math.ceil(this.ref_goban_container.offsetHeight - m.height) / 2,
            left: Math.ceil(this.ref_goban_container.offsetWidth - m.width) / 2,
        });
    }
    onResize = (
        no_debounce: boolean = false,
        skip_state_update: boolean = false,
    ) => {
    ...
    setAnalyzeTool(tool, subtool) {
        if (this.checkAndEnterAnalysis()) {
            ...
            switch (tool) {
                case "draw":
                    this.goban.setAnalyzeTool(
                        tool,
                        this.state.analyze_pencil_color,
                    );
                    break;
                case "erase":
    ...
    toggleCoordinates() {
        ...
        goban.draw_top_labels =
            label_position === "all" || label_position.indexOf("top") >= 0;
        goban.draw_left_labels =
            label_position === "all" || label_position.indexOf("left") >= 0;
        goban.draw_right_labels =
    ...
    updateVariationName(ev) {
        this.setState({
            variation_name: (ev.target as HTMLInputElement).value,
        });
    }
    updateMoveText = (ev) => {
    ...
    popupScores() {
        ...
        if (goban.engine.cur_move) {
            this.orig_marks = JSON.stringify(
                goban.engine.cur_move.getAllMarks(),
            );
        ...
        if (this.goban) {
            ...
            try {
                for (const color in this.goban.engine.players) {
                    if (
                        this.goban.engine.players[color].id ===
                        data.get("user").id
                    ) {
            ...
            new_state.show_submit =
                ...
                goban.engine.cur_move.parent.id ===
                    goban.engine.last_official_move.id;
            new_state.player_to_move = goban.engine.playerToMove();
            new_state.player_not_to_move = goban.engine.playerNotToMove();
            new_state.is_my_move =
                new_state.player_to_move === data.get("user").id;
            new_state.winner = goban.engine.winner;
            new_state.cur_move_number = engine.cur_move
                ? engine.cur_move.move_number
                : -1;
            ...
            new_state.paused =
                goban.pause_control && !!goban.pause_control.paused;
            ...
            new_state.move_text =
                engine.cur_move && engine.cur_move.text
                    ? engine.cur_move.text
                    : "";
                ...
                if (this.return_url && !this.return_url_debounce) {
                    ...
                    setTimeout(() => {
                        if (
                            confirm(
                                interpolate(
                                    _("Would you like to return to {{url}}?"),
                                    { url: this.return_url },
                                ),
                            )
                        ) {
    ...
    createConditionalMoveTreeDisplay(root, cpath, blacks_move) {
        ...
        if (root.move) {
            ...
            ret = [
                $("<span>")
                    ...
                    .append(
                        $("<span>").html(goban.engine.prettyCoords(mv.x, mv.y)),
                    )
                    .addClass(
                        cpath + root.move === goban.getCurrentConditionalPath()
                            ? "selected"
                            : "",
                    )
        ...
        for (const ch in root.children) {
            ...
            const span = $("<span>")
                ...
                .addClass(
                    cpath + ch === goban.getCurrentConditionalPath()
                        ? "selected"
                        : "",
                )
                .click(mkcb(cpath + ch));
    ...
    enterConditionalMovePlanner() {
        ...
        if (
            ...
            this.stashed_conditional_moves =
                this.goban.conditional_tree.duplicate();
    ...
    estimateScore(): boolean {
        ...
        this.goban.setScoringMode(
            true,
            !is_player || this.goban.engine.phase === "finished",
        );
        this.sync_state();
        return true;
    }
    ...
    private annul(tf: boolean): void {
        ...
        do {
            ...
            moderation_note = moderation_note
                ...
                .replace(
                    /(black)\b/g,
                    `player ${this.goban.engine.players.black.id}`,
                )
                .replace(
                    /(white)\b/g,
                    `player ${this.goban.engine.players.white.id}`,
                );
        } while (moderation_note === "");
        ...
        post("games/%%/annul", this.game_id, {
            ...
            .then(() => {
                if (tf) {
                    swal({ text: _("Game has been annulled") }).catch(
                        swal.noop,
                    );
                } else {
                    swal({ text: _("Game ranking has been restored") }).catch(
                        swal.noop,
                    );
                }
    ...
    pass() {
        if (
            ...
            swal({
                text: _("Are you sure you want to pass?"),
                showCancelButton: true,
            })
    ...
    undo() {
        if (
            ...
            this.goban.engine.undo_requested !==
                this.goban.engine.getMoveNumber()
        ) {
            ...
            <div>
                ...
                >
                    ...
                    <div className="center-col">
                        ...
                        {((this.state.view_mode === "portrait" &&
                            !this.state
                                .zen_mode) /* && this.state.portrait_tab === 'chat' */ ||
                            null) &&
                            CHAT}
    ...
    frag_cancel_button() {
        if (this.state.view_mode === "portrait") {
            return (
                <button
                    className="bold cancel-button reject"
                    onClick={this.cancelOrResign}
                >
            ...
            return (
                <button
                    className="xs bold cancel-button"
                    onClick={this.cancelOrResign}
                >
                    {this.state.resign_text}
                </button>
    ...
    frag_play_controls(show_cancel_button) {
        ...
        return (
            <div className="play-controls">
                ...
                {/* } */}
                <div
                    ref={(el) => (this.ref_game_state_label = el)}
                    className="game-state"
                >
                    {/*{*/}
                    {((state.mode === "play" && state.phase === "play") ||
                        null) && (
                        <span>
                            {state.show_undo_requested ? (
                    ...
                    {((state.mode === "play" &&
                        state.phase === "stone removal") ||
                        null) && <span>{_("Stone Removal Phase")}</span>}
                    ...
                    {((state.mode === "play" && state.phase === "finished") ||
                        null) && (
                        <span
                            style={{
                                textDecoration: state.annulled
                                    ? "line-through"
                                    : "none",
                            }}
                        >
                ...
                {(this.state.mode === "conditional" || null) /* { */ && (
                    <div className="conditional-move-planner">
                        <div className="buttons">
                            <button
                                className="primary"
                                onClick={this.acceptConditionalMoves}
                            >
                                {_("Accept Conditional moves")}
                            </button>
                            <button onClick={this.goban_setMode_play}>
                                {_("Cancel")}
                            </button>
                        </div>
                        <div className="ctrl-conditional-tree">
                            ...
                            <PersistentElement
                                elt={this.conditional_move_tree}
                            />
                        </div>
                    </div>
                )}
                ...
                {(state.mode === "score estimation" || null) && (
                    <div className="analyze-mode-buttons">
                        <span>
                            <button
                                className="sm primary bold"
                                onClick={this.stopEstimatingScore}
                            >
                                {_("Back to Board")}
                            </button>
                        </span>
            ...
            <div className="play-controls">
                ...
                >
                    {(this.state.mode === "analyze" || null) && (
                        <div>
                            {_("Review by")}:{" "}
                            <Player user={this.state.review_owner_id} />
                            {((this.state.review_controller_id &&
                                this.state.review_controller_id !==
                                    this.state.review_owner_id) ||
                                null) && (
                                <div>
                                    {_("Review controller")}:{" "}
                                    <Player
                                        user={this.state.review_controller_id}
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-around">
                            {this.state.review_controller_id &&
                                ...
                                this.state.review_out_of_sync && (
                                    <button
                                        className="sm"
                                        onClick={this.syncToCurrentReviewMove}
                                    >
                                        {pgettext(
                                            "Synchronize to current review position",
                                            "Sync",
                                        )}{" "}
            ...
            */}
                ...
                <div className="btn-group">
                    <button
                        ...
                        className={
                            this.state.analyze_tool === "draw" ? "active" : ""
                        }
                    >
                        <i className="fa fa-pencil"></i>
                    </button>
                    <button
                        onClick={this.clearAnalysisDrawing}
                        title={_("Clear pen marks")}
                    >
                ...
                <div className="btn-group">
                    ...
                    <button
                        onClick={this.goban_deleteBranch}
                        title={_("Delete branch")}
                    >
    ...
    frag_ai_review() {
        if (
            ...
            ((this.goban.engine.width === 19 &&
                ...
                (this.goban.engine.width === 13 &&
                    this.goban.engine.height === 13) ||
                (this.goban.engine.width === 9 &&
                    this.goban.engine.height === 9))
        ) {
            return (
                <AIReview
                    onAIReviewSelected={(r) =>
                        this.setState({ selected_ai_review_uuid: r?.uuid })
                    }
                    game={this}
            ...
            return (
                <GameTimings
                    ...
                    free_handicap_placement={
                        this.goban.engine.config.free_handicap_placement
                    }
    ...
    frag_num_captures_text(color) {
        ...
        const prisoner_img_src =
            data.get("config.cdn_release") + "/img/" + prisoner_color + ".png";
        return (
            <div
                className={
                    "captures" + (this.state.estimating_score ? " hidden" : "")
                }
            >
                    ...
                    return (
                        <div key={idx} className={`${color} player-container`}>
                            ...
                            {((goban.engine.players[color] &&
                                ...
                                null) && (
                                <div
                                    className={`${color} player-name-container`}
                                >
                                    <Player
                                        user={
    ...
    frag_below_board_controls() {
        ...
        if (
            this.state.view_mode === "portrait" &&
            this.state.portrait_tab === "dock"
        ) {
            return (
                <div
                    ref={(el) => (this.ref_action_bar = el)}
                    className="action-bar"
                >
                    <span className="move-number">
                        <i
                            onClick={this.togglePortraitTab}
                            className={"tab-icon ogs-goban"}
                        />
                    </span>
                </div>
            );
                ...
                {(this.state.view_mode !== "portrait" || null) && (
                    <span className="move-number">
                        {interpolate(_("Move {{move_number}}"), {
                            move_number:
                                goban && this.goban.engine.getMoveNumber(),
                        })}
                    </span>
                )}
            </div>
    ...
    frag_dock() {
        ...
        let annul =
            ...
            null;
        const annulable =
            (goban && !this.state.annulled && goban.engine.config.ranked) ||
            null;
        const unannulable =
            (goban && this.state.annulled && goban.engine.config.ranked) ||
            null;
        ...
        try {
            sgf_download_enabled =
                this.goban.engine.phase === "finished" ||
                !this.goban.isAnalysisDisabled(true);
        ...
        return (
            <Dock>
                {(this.tournament_id || null) && (
                    <Link
                        className="plain"
                        to={`/tournament/${this.tournament_id}`}
                    >
                        <i
                            className="fa fa-trophy"
                            title={_("This is a tournament game")}
                        />{" "}
                        {_("Tournament")}
                    </Link>
                )}
                {(this.ladder_id || null) && (
                    <Link className="plain" to={`/ladder/${this.ladder_id}`}>
                        <i
                            className="fa fa-trophy"
                            title={_("This is a ladder game")}
                        />{" "}
                        {_("Ladder")}
                    </Link>
                )}
                {((this.goban && this.goban.engine.config["private"]) ||
                    null) && (
                    ...
                    annul &&
                        unannulable && (
                            <a onClick={() => this.annul(false)}>
                                <i className="fa fa-gavel unannulable"></i>{" "}
                                {"Remove annulment"}
                            </a>
                ...
                }
                {
                    annul &&
                        ...
                        !unannulable && (
                            <div>
                                <i className="fa fa-gavel greyed"></i>{" "}
                                {_("Annul")}
                            </div>
                        ) /* What is this case?! */
                }
    ...
    frag_kb_shortcuts() {
        ...
        return (
            <div>
                ...
                <KBShortcut
                    shortcut="f1"
                    action={this.set_analyze_tool.stone_null}
                />
                <KBShortcut
                    shortcut="f2"
                    action={this.set_analyze_tool.stone_black}
                />
                {/* <KBShortcut shortcut='f3' action='console.log("Should be entering scoring mode");'></KBShortcut> */}
                <KBShortcut
                    shortcut="f4"
                    action={this.set_analyze_tool.label_triangle}
                />
                <KBShortcut
                    shortcut="f5"
                    action={this.set_analyze_tool.label_square}
                />
                <KBShortcut
                    shortcut="f6"
                    action={this.set_analyze_tool.label_circle}
                />
                <KBShortcut
                    shortcut="f7"
                    action={this.set_analyze_tool.label_letters}
                />
                <KBShortcut
                    shortcut="f8"
                    action={this.set_analyze_tool.label_numbers}
                />
                ...
                {((goban && goban.mode === "analyze") || null) && (
                    <KBShortcut
                        shortcut="f10"
                        action={this.set_analyze_tool.clear_and_sync}
                    />
                )}
                <KBShortcut
                    shortcut="del"
                    action={this.set_analyze_tool.delete_branch}
                />
            </div>
        );
    }
    ...
    renderExtraPlayerActions = (player_id: number, unused: any) => {
        ...
        if (
            this.review_id &&
            this.goban &&
            (this.goban.review_controller_id === user.id ||
                this.goban.review_owner_id === user.id)
        ) {
            let is_owner = null;
    ...
    setMoveTreeContainer = (resizable: Resizable): void => {
        ...
        if (this.goban) {
            (this.goban as GobanCanvas).setMoveTreeContainer(
                this.ref_move_tree_container,
            );
        }
    };
}
