...
export class AIReview extends React.Component<
    AIReviewProperties,
    AIReviewState
> {
    ...
    syncAIReview() {
        ...
        for (
            let move_number = 0;
            move_number < ai_review.win_rates.length;
            ++move_number
        ) {
    ...
    startNewAIReview(
        analysis_type: "fast" | "full",
        engine: "leela_zero" | "katago",
    ) {
            ...
            for (const k in ai_review) {
                ...
                if (k !== "moves" || !this.ai_review["moves"]) {
                    ...
                    for (const move in ai_review["moves"]) {
                        this.ai_review["moves"][move] =
                            ai_review["moves"][move];
                    }
                }
            }
        }
                ...
                for (const key in data) {
                    ...
                    if (key === "metadata") {
                        ...
                        if (this.ai_review) {
                            ...
                            console.error(
                                "AI Review missing, cannot update error",
                                value,
                            );
                        }
                        ...
                        console.warn(
                            `Unrecognized key in updateAiReview data: ${key}`,
                            value,
                        );
                    }
                }
        ...
        try {
            if ((cur_move.trunk || have_variation_results) && ai_review_move) {
                ...
                for (const branch of branches) {
                    ...
                    if (
                        next_move &&
                        isEqualMoveIntersection(branch.moves[0], next_move)
                    ) {
                ...
                for (let y = 0; y < this.props.game.goban.engine.height; y++) {
                    ...
                    for (
                        let x = 0;
                        x < this.props.game.goban.engine.width;
                        x++
                    ) {
                        r.push(0);
                    }
                ...
                for (let i = 0; i < branches.length; ++i) {
                    ...
                    if (
                        ...
                        if (
                            parseFloat(key).toPrecision(2).length < key.length
                        ) {
                            key = parseFloat(key).toPrecision(2);
                        }
                        this.props.game.goban.setSubscriptMark(
                            mv.x,
                            mv.y,
                            key,
                            true,
    ...
    private requestAnalysisOfVariation(
        cur_move: MoveTree,
        trunk_move: MoveTree,
    ): boolean {
        if (!this.props.game) {
    ...
    private fillAIMarksBacktracking(
        ...
        for (let j = 0; j <= trunk_move.move_number; j++) {
            ...
            const ai_review_move =
                this.ai_review.moves[trunk_move.move_number - j];
            if (!ai_review_move) {
                continue;
            }
            ...
            trunk_move_string = trunk_move_string.slice(
                0,
                trunk_move_string.length - 2 * j,
            );
            ...
            for (const branch of ai_review_move.branches) {
                const move_str: string =
                    trunk_move_string + GoMath.encodeMoves(branch.moves);
                if (move_str.startsWith(cur_move_string)) {
                    next_moves = move_str.slice(
                        cur_move_string.length,
                        Infinity,
                    );
                    break;
                }
            ...
            if (next_moves) {
                const decoded_moves =
                    this.props.game.goban.engine.decodeMoves(next_moves);
                let black = "";
                let white = "";
                ...
                for (let i = 0; i < decoded_moves.length; ++i) {
                    ...
                    marks[i + cur_move.getDistance(trunk_move) + 1] =
                        encoded_mv;
                    if (
                        (this.props.game.goban.engine.player - 1 + i) % 2 ===
                        1
                    ) {
                        white += encoded_mv;
                    } else {
    ...
    private AiSummaryTableRowList() {
        ...
        const other_game_type =
            this.props.game.goban.engine.initial_state.white !== "";
        ...
        if (this.ai_review?.type === "fast") {
            ...
            const check1 =
                !is_uploaded &&
                this.props.game.goban.config.moves.length !==
                    this.ai_review?.scores.length - 1;
            ...
            const check2 =
                is_uploaded &&
                this.props.game.goban.config["all_moves"].split("!").length -
                    bplayer !==
            ...
            for (let j = hoffset; j < scores.length - 1; j++) {
                ...
                const is_bplayer =
                    move_player_list[j] === JGOFNumericPlayerColor.BLACK;
            ...
            avg_score_loss[0] =
                btotal > 0
                    ? Math.round((10 * avg_score_loss[0]) / btotal) / 10
                    : 0;
            avg_score_loss[1] =
                wtotal > 0
                    ? Math.round((10 * avg_score_loss[1]) / wtotal) / 10
                    : 0;
            ...
            for (let j = 0; j < num_rows; j++) {
                ...
                summary_moves_list[j][1] =
                    btotal > 0
                        ? (
                              Math.round((1000 * movecounters[j]) / btotal) / 10
                          ).toString()
                        : "";
                summary_moves_list[j][2] =
                    movecounters[num_rows + j].toString();
                summary_moves_list[j][3] =
                    wtotal > 0
                        ? (
                              Math.round(
                                  (1000 * movecounters[num_rows + j]) / wtotal,
                              ) / 10
                          ).toString()
                        : "";
            }
            ...
            for (let j = hoffset; j < this.ai_review?.scores.length - 1; j++) {
                ...
                const current_branches = this.ai_review?.moves[
                    j
                ].branches.slice(0, 6);
                ...
                const player_index = is_bplayer ? 0 : 1;
                let scorediff =
                    this.ai_review?.moves[j + 1].score -
                    this.ai_review?.moves[j].score;
                scorediff = is_bplayer ? -1 * scorediff : scorediff;
                avg_score_loss[player_index] += scorediff;
    ...
    public render(): JSX.Element {
        ...
        const ai_review_chart_entries: Array<AIReviewEntry> =
            this.ai_review.win_rates?.map((x, idx) => {
                return {
                    ...
                    num_variations:
                        this.ai_review?.moves[idx]?.branches.length || 0,
                };
            }) || [];
        ...
        const variation_move_number =
            cur_move.move_number !== trunk_move.move_number
                ? cur_move.move_number
                : -1;
        const worst_move_list = getWorstMoves(
        ...
        return (
            <div className="AIReview">
                ...
                {this.ai_review.error ? (
                    ...
                    <React.Fragment>
                        {((this.ai_review && this.ai_review.win_rates) ||
                            null) && (
                            <React.Fragment>
                                <AIReviewChart
                                    ...
                                    variation_entries={
                                        ai_review_chart_variation_entries
                                    }
                ...
                {null &&
                    next_move_pretty_coords &&
                    next_move_delta_win_rate !== null && (
                        <div className="next-move-delta-container">
                            <span
                                className={
                                    "next-move-coordinates " +
                                    (this.props.game.goban.engine.colorToMove() ===
                                    "white"
                                        ? "white-background"
                                        : "black-background")
                                }
        ...
        return (
            <div className="worst-move-list-container">
                <div className="move-list">
                    {pgettext(
                        "Moves that were the biggest mistakes, according to the AI",
                        "Key moves",
                    )}
                    :
                    {lst.slice(0, 3).map((de, idx) => {
                        const pretty_coords =
                            this.props.game.goban.engine.prettyCoords(
                                de.move.x,
                                de.move.y,
                            );
                        return (
...
function sanityCheck(ai_review: JGOFAIReview) {
    if (ai_review.moves["0"]) {
        if (ai_review.moves["0"].move.x !== -1) {
            console.error(
                "AI Review move '0' is not a pass move, was ",
                ai_review.moves["0"].move,
            );
        }
    }
    if (typeof ai_review.moves !== "object") {
        console.error(
            "AI Review moves was not an object",
            JSON.stringify(ai_review.moves),
        );
    }
}

function isEqualMoveIntersection(
    a: JGOFIntersection,
    b: JGOFIntersection,
): boolean {
...
function ReviewStrengthIcon({ review }: { review: JGOFAIReview }): JSX.Element {
    ...
    return (
        <span className={"ai-review-strength-icon " + strength}>{content}</span>
    );
}
...
class AiSummaryTable extends React.Component<
    AiSummaryTableProperties,
    AiSummaryTableState
> {
        ...
        return (
            <div className="ai-summary-container">
                <table
                    ...
                    style={{
                        display: this.props.table_hidden ? "block" : "none",
                    }}
                >
                    <thead>
                        <tr>
