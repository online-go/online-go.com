...
export class GameTimings extends React.Component<GameTimingProperties> {
    ...
    public render(): JSX.Element {
        const game_elapsed: ReturnType<typeof moment.duration> =
            moment.duration(0); // running total
        const black_elapsed: ReturnType<typeof moment.duration> =
            moment.duration(0);
        const white_elapsed: ReturnType<typeof moment.duration> =
            moment.duration(0);
        ...
        if (this.props.handicap) {
            if (this.props.free_handicap_placement) {
                ...
                handicap_moves = this.props.moves.slice(
                    0,
                    this.props.handicap - 1,
                );
                non_handicap_moves = this.props.moves.slice(
                    this.props.handicap - 1,
                );
                    ...
                    non_handicap_moves
                        ...
                        .reduce((acc, value, index, orig) => {
                            if (index % 2 === 0) {
                                ...
                                for (const player_id of non_handicap_moves?.[
                                    index
                                ]?.[4]?.sgf_downloaded_by || []) {
                                    if (
