...
export class GameLogModal extends Modal<
    Events,
    GameLogModalProperties,
    { log: Array<LogEntry> }
> {
    constructor(props) {
        super(props);
        ...
        termination_socket.send(`game/log`, { game_id }, (log) =>
            this.setLog(log),
        );
    }
        ...
        try {
            for (const k in data) {
                if (k === "player_id") {
                    ...
                    const stones = GoMath.decodeMoves(
                        data[k],
                        config.width,
                        config.height,
                    )
                        .map((mv) =>
                            GoMath.prettyCoords(mv.x, mv.y, config.height),
                        )
                        .join(", ");
