...
/* Chat  */
export class GameChat extends React.PureComponent<
    GameChatProperties,
    GameChatState
> {
    ref_chat_log;
    ...
    onKeyPress(event) {
        if (event.charCode === 13) {
            if (event.target.className === "qc-option") {
                ...
                this.props.gameview.goban.sendChat(
                    input.value,
                    this.state.chat_log,
                );
                input.value = "";
    ...
    updateScrollPosition() {
        ...
        if (tf !== this.scrolled_to_bottom) {
            ...
            this.ref_chat_log.className =
                "chat-log " + (tf ? "autoscrolling" : "");
        }
        ...
        if (this.scrolled_to_bottom) {
            ...
            setTimeout(() => {
                if (this.ref_chat_log) {
                    this.ref_chat_log.scrollTop =
                        this.ref_chat_log.scrollHeight;
                }
            }, 100);
        }
    }
    toggleChatLog = () => {
        const new_chat_log =
            this.state.chat_log === "main" ? "malkovich" : "main";
        this.setState({
    ...
    toggleModeratorChatLog = () => {
        const new_chat_log =
            this.state.chat_log === "main" ? "moderator" : "main";
        this.setState({
            chat_log: new_chat_log,
            qc_visible: false,
            qc_editing: false,
    ...
    saveEdit = () => {
        ...
        localStorage.setItem(
            "ogs.qc.messages",
            JSON.stringify(user.qc_phrases),
        );
        this.finishEdit();
    };
            ...
            <div className="chat-container">
                ...
                <div className="chat-input-container input-group">
                    ...
                    <TabCompleteInput
                        ...
                        disabled={
                            user.anonymous || !data.get("user").email_validated
                        }
                        placeholder={
                            user.anonymous
                                ? _("Sign in to chat")
                ...
                <li
                    ...
                    ref={
                        this.state.qc_editing
                            ? (input) => {
                                  this.qc_editableMsgs =
                                      index === 0 ? [] : this.qc_editableMsgs;
...
function parsePosition(position: string) {
    ...
    if (
        i >= ((goban && goban.width) || 19) ||
        j >= ((goban && goban.height) || 19)
    ) {
        i = -1;
        j = -1;
    }
...
export class GameChatLine extends React.Component<GameChatLineProperties> {
                    ...
                    case "analysis": {
                        ...
                        const onLeave = () => {
                            if (this.props.gameview.in_pushed_analysis) {
                                ...
                                this.props.gameview.leave_pushed_analysis =
                                    null;
    ...
    render() {
        ...
        if (
            ...
            line.body.substr(0, 4) === "/me "
        ) {
            third_person =
                line.body.substr(0, 4) === "/me " ? "third-person" : "";
            ...
            if (line.date) {
                if (
                    ...
                    moment(new Date(line.date * 1000)).format("YYYY-MM-DD") !==
                        moment(new Date(lastline.date * 1000)).format(
                            "YYYY-MM-DD",
                        )
                ) {
                    show_date = (
        ...
        return (
            <div className={`chat-line-container`} data-chat-id={chat_id}>
                ...
                >
                    ...
                    {(line.player_id || null) && (
                        <Player user={line} flare disableCacheUpdate />
                    )}
                    <span className="body">
                        {third_person ? " " : ": "}
                        {msg}
