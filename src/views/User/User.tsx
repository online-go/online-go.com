...
export class User extends React.PureComponent<UserProperties, UserState> {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            show_ratings_in_rating_grid: preferences.get(
                "show-ratings-in-rating-grid",
            ),
            rating_graph_plot_by_games: preferences.get(
        ...
        try {
            this.show_mod_log =
                parse(this.props.location.search)["show_mod_log"] === "1";
        } catch (e) {
            ...
            if (this.state.user) {
                if (this.state.user.on_vacation) {
                    const time_diff = Math.round(
                        (Date.now() - interval_start) / 1000,
                    );
                    const vacation_time_left =
                        this.state.user.vacation_left - time_diff;
                    this.setState({
                        vacation_left_text:
    ...
    vacationAccrued() {
        if (this.state.user) {
            ...
            if (this.isSpecialUser()) {
                return (
                    daysOnlyDurationString(vacation_time_accrued) +
                    " " +
                    _("out of 60 Days")
                );
            } else {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        if (
            nextProps.match.params.user_id !== this.props.match.params.user_id
        ) {
    ...
    resolve(props) {
        ...
        this.user_id = parseInt(
            props.match.params.user_id || data.get("user").id,
        );
    ...
    update(state) {
        ...
        state.user.rating_correspondence = parseFloat(
            state.user.rating_correspondence,
        );
        ...
        for (let i = 0; i < state.vs.recent5.length; ++i) {
            state.vs.recent5[i].pretty_date = moment(
                new Date(state.vs.recent5[i].date),
            ).format("ll");
        ...
        if (state.user.professional) {
            for (let i = 37; i < Math.max(state.user.ranking, 45) + 1; ++i) {
                state.ranks.push({
                    value: i,
                    text: longRankString({ ranking: i, pro: 1 }),
                });
            }
        } else {
    ...
    saveHostIpSettings() {
        ...
        const obj = {
            ...
            ban_affects_all: this.state.host_ip_settings.ban_affects_all
                ? 1
                : 0,
            chatban_affects_all: this.state.host_ip_settings.chatban_affects_all
                ? 1
                : 0,
        };
        console.log("->", obj);
    ...
    saveSuperUserStuff() {
        ...
        put("players/%%/moderate", this.user_id, {
            ...
            ranking_correspondence: parseInt(
                $("#user-su-ranking-correspondence").val(),
            ),
            ...
            can_create_tournaments: $("#user-su-can-create-tournaments").is(
                ":checked",
            )
                ? 1
                : 0,
            locked_username: $("#user-su-locked-username").is(":checked")
                ? 1
                : 0,
    ...
    toggleRatings = () => {
        this.setState((state) => ({
            temporary_show_ratings: !state.temporary_show_ratings,
        }));
    };
    saveCountry = (ev) => {
        this.setState({
    ...
    saveRealLastName = (ev) => {
        this.setState({
            user: Object.assign({}, this.state.user, {
                ...
                name:
                    (this.state.user.first_name || "") + " " + ev.target.value,
            }),
        });
    };
    saveRealNameIsPrivate = (ev) => {
        this.setState({
            user: Object.assign({}, this.state.user, {
                real_name_is_private: ev.target.checked,
            }),
        });
    };
    saveEditChanges() {
    ...
    render() {
        ...
        const game_history_groomer = (results) => {
            ...
            for (let i = 0; i < results.length; ++i) {
                ...
                if ("time_control_parameters" in r) {
                    ...
                    if (tcp && "speed" in tcp) {
                        item.speed =
                            tcp.speed[0].toUpperCase() + tcp.speed.slice(1); // capitalize string
                    }
                }
                if (!item.speed) {
                    // fallback
        ...
        return (
            <div className="User container">
                        ...
                        <div className="avatar-and-ratings-row">
                            <div className="avatar-container">
                                ...
                                <div className="avatar-subtext">
                                    ...
                                    {user.ui_class_extra &&
                                        user.ui_class_extra.indexOf("aga") >=
                                            0 && (
                                            <div>
                                ...
                                {editing ? (
                                    ...
                                    <div className="country-line">
                                        <Flag country={user.country} big />
                                        <span>
                                            {cc_to_country_name(user.country)}
                                        </span>
                                    </div>
                                )}
                ...
                {preferences.get("hide-ranks") && (
                    <button
                        className="danger toggle-ratings"
                ...
                <div className="row">
                    <div className="col-sm-8">
                        {((window["user"] && window["user"].is_moderator) ||
                            null) && (
                            <Card>
                                {" "}
                                {/* Moderator stuff  */}
                                ...
                                <UIPush
                                    ...
                                    channel="moderators"
                                    action={() =>
                                        this.moderator_log_table_ref.current?.refresh()
                                    }
                                />
                                <div
                                    id="leave-moderator-note"
                                    ref={this.moderator_log_anchor}
                                >
                                    <textarea
                                        ...
                                        id="moderator-note"
                                    />
                                    <button onClick={this.addModeratorNote}>
                                        Add note
                                    </button>
                                </div>
                                <PaginatedTable
                        ...
                        {(this.state.active_games.length > 0 || null) && (
                            <h2>
                                {_("Active Games")} (
                                {this.state.active_games.length})
                            </h2>
                        )}
                        <GameList
                            list={this.state.active_games}
                            player={user}
                        />
                                ...
                                <Card>
                                    <div>
                                        ...
                                        <PaginatedTable
                                            ...
                                            onRowClick={(ref, ev) =>
                                                openUrlIfALinkWasNotClicked(
                                                    ev,
                                                    ref.href,
                                                )
                                            }
                                            columns={[
                                                /* I wish we could set properties at the row level! */
                                                {
                                                    header: _("User"),
                                                    className: (X) =>
                                                        "user_info" +
                    ...
                    <div className="col-sm-4">
                        ...
                        {user.is_bot &&
                            user.bot_owner &&
                            user.bot_owner.id === window["user"].id && (
                                <div>
                        ...
                        {this.state.ip && (
                            <Card>
                                ...
                                <div>
                                    <b>Region</b>
                                    {this.state.ip.subdivisions.map(
                                        (sd, idx) => (
                        ...
                        <Card className="activity-card">
                            ...
                            {!user.on_vacation && (
                                <div>{this.vacationAccrued()}</div>
                            )}
                            {user.on_vacation && (
                                <div>{_("User On Vacation")}</div>
                            )}
                            <h4>{_("Ladders")}</h4>
                            {this.state.ladders.length > 0 && (
                                <div>
                                    <dl className="activity-dl">
                                        {this.state.ladders.map(
                                            (ladder, idx) => (
                                                <dd key={idx}>
                                                    #{ladder.rank}{" "}
                                                    <Link
                            ...
                            {!this.state.ladders.length && (
                                <div>
                                    <div>
                                        {_("Not participating in any ladders")}
                                    </div>
                                </div>
                            )}
        ...
        return (
            <div className="ratings-grid">
                <div className="title-row">
                    ...
                    <span className="title">
                        <i
                            className="speed-icon fa fa-circle-o"
                            title={_("Overall")}
                        />
                    </span>
                    <span className="title">
                        <i
                            className="speed-icon fa fa-bolt"
                            title={_("Blitz")}
                        />
                    </span>
                    <span className="title">
                        <i
                            className="speed-icon fa fa-clock-o"
                            title={_("Live")}
                        />
                    </span>
                    <span className="title">
                        <i
                            className="speed-icon ogs-turtle"
                            title={_("Correspondence")}
                        />
                    </span>
                </div>
                {[0, 9, 13, 19].map((size) => (
                ...
                <div className="rating">
                    <span className="left">
                        {show_rating
                            ...
                            : boundedRankString(
                                  rating_to_rank(
                                      humble_rating(r.rating, r.deviation),
                                  ),
                                  true,
                              )}
                    </span>
    ...
    return (
        <div className="SelfReportedAccountLinkages">
            ...
            <AssociationLink
                country={links.org1}
                id={links.org1_id}
                rank={links.org1_rank}
            />
            <AssociationLink
                country={links.org2}
                id={links.org2_id}
                rank={links.org2_rank}
            />
            <AssociationLink
                country={links.org3}
                id={links.org3_id}
                rank={links.org3_rank}
            />

            {has_other_server && <h3>{_("Servers")}</h3>}
            <ServerLink
                name={_("KGS")}
                id={links.kgs_username}
                rank={links.kgs_rank}
            />
            <ServerLink
                name={_("IGS / PandaNet")}
                id={links.igs_username}
                rank={links.igs_rank}
            />
            <ServerLink
                name={_("DGS")}
                id={links.dgs_username}
                rank={links.dgs_rank}
            />
            <ServerLink
                name={_("Little Golem")}
            ...
            <ServerLink
                name={_("WBaduk")}
                id={links.wbaduk_username}
                rank={links.wbaduk_rank}
            />
            <ServerLink
                name={_("Tygem")}
                id={links.tygem_username}
                rank={links.tygem_rank}
            />
            <ServerLink
                name={_("Fox")}
                id={links.fox_username}
                rank={links.fox_rank}
            />
            <ServerLink
                name={_("Yike Weiqi")}
                id={links.yike_username}
                rank={links.yike_rank}
            />
            <ServerLink
                name={_("GoQuest")}
                id={links.goquest_username}
                rank={links.goquest_rank}
            />
        </div>
    );
}
function AssociationLink({
    ...
    try {
        ...
        const association = associations.filter(
            (a) => a.country === country,
        )[0];
