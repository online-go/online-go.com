...
export class TournamentListMainView extends React.PureComponent<
    {},
    TournamentListMainViewState
> {
    constructor(props) {
        super(props);
        this.state = {
...
function rrule_description(entry) {
    const m = moment(new Date(entry.next_run)).add(
        entry.lead_time_seconds,
        "seconds",
    );
    ...
    if (interval === 1) {
        switch (unit) {
            case "hourly":
                return m.format("m") !== "0"
                    ? interpolate(
                          _("Occurs %s minutes past the hour every hour"),
                          [m.format("m")],
                      )
        ...
        switch (unit) {
            ...
            case "hourly":
                return m.format("m") !== "0"
                    ? interpolate(
                          _("Occurs %s minutes past the hour every %s hours"),
                          [m.format("m"), interval],
                      )
                    : interpolate(_("Occurs every %s hours on the hour"), [
                          interval,
                      ]);
            case "daily":
