...
try {
    ...
    if (primary_language === true) {
        global_channels[0].primary_language = true;
    }
...
class ChatChannel extends TypedEventEmitter<Events> {
    ...
    constructor(channel: string, display_name: string) {
        ...
        this.joining = true;
        setTimeout(() => (this.joining = false), 10000);
...
export class ChatChannelProxy extends TypedEventEmitter<Events> {
    ...
    _onChat = (...args: any[]) => {
        // Spread args so listeners receive the original payload, not [payload] (#3534)
        this.emit.apply(this, ["chat", ...args] as [string, ...any[]]);
    };
    _onTopic = (...args: any[]) => {
        this.emit.apply(this, ["topic", ...args] as [string, ...any[]]);
    };
    _onJoin = (...args: any[]) => {
        this.emit.apply(this, ["join", ...args] as [string, ...any[]]);
    };
    _onPart = (...args: any[]) => {
        this.emit.apply(this, ["part", ...args] as [string, ...any[]]);
    };
    ...
    _onChatRemoved = (...args: any[]) => {
        this.emit.apply(this, ["chat-removed", ...args] as [string, ...any[]]);
    };
    _onUnreadChanged = (...args: any[]) => {
        this.emit.apply(this, ["unread-count-changed", ...args] as [string, ...any[]]);
    };
    _destroy() {
