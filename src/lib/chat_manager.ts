...
export class ChatChannelProxy extends TypedEventEmitter<Events> {
    ...
    _onChat = (...args: any[]) => {
        // Spread so listeners get the original payload, not [payload] (#3534)
        this.emit.apply(this, ["chat", ...args] as any);
    };
    _onTopic = (...args: any[]) => {
        this.emit.apply(this, ["topic", ...args] as any);
    };
    _onJoin = (...args: any[]) => {
        this.emit.apply(this, ["join", ...args] as any);
    };
    _onPart = (...args: any[]) => {
        this.emit.apply(this, ["part", ...args] as any);
    };
    ...
    _onChatRemoved = (...args: any[]) => {
        this.emit.apply(this, ["chat-removed", ...args] as any);
    };
    _onUnreadChanged = (...args: any[]) => {
        this.emit.apply(this, ["unread-count-changed", ...args] as any);
    };
    _destroy() {
