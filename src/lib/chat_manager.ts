...
const channel_information_resolvers: {
    [channel: string]: Promise<ChannelInformation>;
} = {};
...
try {
    ...
    if (primary_language === true) {
        // not found
        global_channels[0].primary_language =
            true; /* default to english as primary */
    }
...
data.watch("user", (user) => {
    if (
        user.supporter &&
        global_channels.filter((c) => c.id === "global-supporter").length === 0
    ) {
    ...
    if (
        user.is_moderator &&
        global_channels.filter((c) => c.id === "shadowban").length === 0
    ) {
        global_channels.splice(0, 0, {
            id: "shadowban",
            country: "_Pirate",
...
export function resolveChannelDisplayName(channel: string): string {
    ...
    if (channel.startsWith("global-")) {
        ...
        return interpolate(_("Game {{number}}"), {
            number: channel.substring(5),  // eslint-disable-line
        });
    } else if (channel.startsWith("review-")) {
        return interpolate(_("Review {{number}}"), {
            number: channel.substring(7),  // eslint-disable-line
        });
    }
    return "<error>";
}
...
data.watch("config.user", (user) => {
    const cleaned_username_regex = user.username.replace(
        /[\\^$*+.()|[\]{}]/g,
        "\\$&",
    );
...
export function cachedChannelInformation(
    channel: string,
...
export function updateCachedChannelInformation(
    channel: string,
...
export function resolveChannelInformation(
    channel: string,
