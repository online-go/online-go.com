...
export function get_handicap_adjustment(
    rating: number,
    handicap: number,
): number {
...
export function getUserRating(
    ...
    ret.rank_deviation =
        rating_to_rank(ret.rating + ret.deviation) - rating_to_rank(ret.rating);
    ...
    ret.partial_bounded_rank = Math.max(
        MinRank,
        Math.min(MaxRank, ret.partial_rank),
    );
...
export function effective_outcome(
    ...
    return {
        ...
        black_effective_stronger:
            black_effective_rating > white_effective_rating,
        white_real_stronger: white_rating >= black_rating,
        white_effective_stronger:
            white_effective_rating >= black_effective_rating,
    };
}
