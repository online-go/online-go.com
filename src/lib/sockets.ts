...
} else if (window.OGS_DEV_BACKEND === "LOCAL") {
    // if we're a developer using a local server, then use it for ai
    ai_host = `http://localhost:13284`;
} else if (window.OGS_DEV_BACKEND === "PRODUCTION") {
    // if we're a developer connecting to the production system, use the production ai server
    ai_host = `https://ai.online-go.com`;
} else if (
    // web developers who are _not_ using a local server use Beta for AI
    window.OGS_DEV_BACKEND === "BETA" ||
