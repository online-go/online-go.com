...
export let current_language: string =
    (w["ogs_current_language"] as string) || "en";
//export let languages: { [key: string]: string } = w["ogs_languages"] || { 'en': 'English' };
export const languages: { [key: string]: string } = w[
    "supported_languages"
] || { en: "English" };
export const countries: { [key: string]: { [key: string]: string } } = w[
    "ogs_countries"
] || { en: { us: "United States" } };
export const locales: { [key: string]: { [key: string]: string[] } } = w[
    "ogs_locales"
] || { en: {} };
    ...
    switch (current_language) {
        ...
        case "sr": // Serbian
            pluralidx = (count: number) => {
                ...
                if (
                    isInRange(count % 10, 2, 4) &&
                    !isInRange(count % 100, 12, 14)
                ) {
                    return 1;
                } // 2, 3, 4, 22, 23, 24...
...
const debug_wrap =
    current_language === "debug" ? (s: string) => `[${s}]` : (s: string) => s;
...
fantasy_countries.push([
    "_United_Federation_of_Planets",
    gettext("United Federation of Planets"),
]);
    ...
    if (typeof params === "object") {
        return str.replace(/{{([^}]+)}}/g, (_, key, position) => {
            if (!(key in params)) {
                ...
                console.warn(
                    `Missing interpolation key: ${key} for string: ${str}`,
                );
            }
            return params[key];
        });
...
export function getLanguageFlag(
    ...
    if (
        ...
        [
            ...
            "ie",
            "us",
        ].indexOf(user_country) >= 0
    ) {
        return sanitize(user_country);
    }
    if (
        language === "spanish" &&
        ["mx", "co", "cl", "ar"].indexOf(user_country) >= 0
    ) {
        return sanitize(user_country);
    }
    if (
        language === "french" &&
        ["ca", "be", "cd", "ci", "ch"].indexOf(user_country) >= 0
    ) {
        return sanitize(user_country);
    }
    if (
        language === "german" &&
        ["at", "de", "be", "ch"].indexOf(user_country) >= 0
    ) {
        return sanitize(user_country);
    }
    if (
        language === "italian" &&
        ["it", "ch", "va", "sm"].indexOf(user_country) >= 0
    ) {
        return sanitize(user_country);
    }
    if (
        language === "portuguese" &&
        ["pt", "br", "mz", "ao"].indexOf(user_country) >= 0
    ) {
        return sanitize(user_country);
    }
...
export function setCurrentLanguage(language_code: string) {
    ...
    setGobanTranslations({
        ...
        "A stone has already been placed here": _(
        ...
        "Synchronization error, reloading": _(
