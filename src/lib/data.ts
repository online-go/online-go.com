...
let wal: {
    [key: string]: { key: string; value?: any; replication: Replication };
} = {};
...
function remote_set(
    ...
    if (
        remote_store[key]?.value === value &&
        remote_store[key]?.replication === replication
    ) {
        return;
    }
...
function _enqueue_set(
    ...
    safeLocalStorageSet(
        `ogs-remote-storage-wal.${user_id}.${key}`,
...
function _process_write_ahead_log(user_id: number): void {
    for (const data_key in wal) {
        ...
        const cb = (res) => {
            ...
            if (
                wal[data_key].value !== kv.value ||
                wal[data_key].replication !== kv.replication
            ) {
                ...
                safeLocalStorageRemove(
                    `ogs-remote-storage-wal.${user_id}.${kv.key}`,
                );
                delete wal[kv.key];
                remote_sync();
            }
...
termination_socket.on("remote_storage/update", (row: RemoteKV) => {
    ...
    safeLocalStorageSet(
        `ogs-remote-storage-store.${user.id}.${row.key}`,
        JSON.stringify(row),
    );

    if (last_modified < row.modified) {
        safeLocalStorageSet(
            `ogs-remote-storage-last-modified.${user.id}`,
            row.modified,
        );
        ...
        for (let i = 0; i < localStorage.length; ++i) {
            ...
            if (full_key.indexOf(store_prefix) === 0) {
                ...
                try {
                    ...
                    console.error(
                        `Error loading remote storage key ${full_key}, removing`,
                        e,
                    );
            ...
            if (full_key.indexOf(wal_prefix) === 0) {
                ...
                try {
                    ...
                    console.error(
                        `Error loading WAL key ${full_key}, removing`,
                        e,
                    );
                    localStorage.removeItem(full_key);
                }
