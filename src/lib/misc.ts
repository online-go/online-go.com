...
export function getOutcomeTranslation(outcome: string) {
    ...
    if (/[0-9.]+/.test(outcome)) {
        ...
        return interpolate(pgettext("Game outcome", "{{number}} points"), {
            number: num,  // eslint-disable-line
        });
    }
...
export function string_splitter(
    str: string,
    ...
    for (const pieces of arr) {
        ...
        for (const s of pieces) {
            if (
                cur.length + s.length + (cur.length === 0 ? 0 : 1) <
                max_length
            ) {
                cur += (cur.length ? " " : "") + s;
            } else {
    ...
    swal({
        ...
        .then((description) => {
            ...
            post("moderation/incident", obj)
                .then(() => {
                    swal({ text: _("Thanks for the report!") }).catch(
                        swal.noop,
                    );
                })
                .catch(errorAlerter);
...
export function shouldOpenNewTab(event) {
    ...
    if (
        event &&
        (event.which === 2 || event.metaKey || event.ctrlKey || event.shiftKey)
    ) {
        return true;
    }
    /*
...
export function navigateTo(path, event?) {
    if (
        ...
        Date.now() - last_navigateTo.timestamp < 100
    ) {
...
export function ogs_has_focus() {
    try {
        return (
            document.hasFocus() ||
            localStorage.getItem("focused_window") != null
        );
    } catch (e) {
