...
export class SFXSprite {
    ...
    public play(repeat_breaker_ms?: number): void {
        ...
        if (
            repeat_breaker_ms &&
            Date.now() - this.last_time_played < repeat_breaker_ms
        ) {
            console.log(
                "*NOT* Playing sound bite:",
                this.name,
...
export class SFXManager {
    ...
    public play(
        sound_name: ValidSound,
        repeat_breaker_ms?: number,
    ): SFXSprite | null {
        try {
            ...
            if (
                this.getSpriteVoiceEnabled(sound_name) &&
                sound_name in this.sprites
            ) {
        ...
        try {
            if (group_name) {
                if (group_name in this.howls) {
                    ...
                    throw new Error(
                        `Called sfx.stop with invalid group name: ${group_name}`,
                    );
                }
            } else {
    ...
    public load(group_name: ValidSoundGroup): void {
        ...
        */
        ...
        const howl = new Howl({
            ...
            autoplay: false,
            sprite: sprite_pack.definitions as {
        ...
        for (const sprite_name in sprite_pack.definitions) {
            if (sound_list.indexOf(sprite_name as ValidSound) >= 0) {
                this.sprites[sprite_name] = new SFXSprite(
                    howl,
                    group_name,
                    sprite_name,
                );
        ...
        if (group_name === "effects") {
            for (const sprite_name in sprite_pack.definitions) {
                this.effectSprites[sprite_name] = new SFXSprite(
                    howl,
                    group_name,
                    sprite_name,
                );
                this.effectSprites[sprite_name].volume =
                    this.getVolume(group_name);
            }
        }
    ...
    public getPackId(group_name: ValidSoundGroup): string {
        ...
        for (const l of to_check) {
            if (l.indexOf("en") === 0) {
                ...
                if (
                    group_name === "countdown" &&
                    "en-gb-claire-numbers" in sprite_packs
                ) {
                ...
                if (
                    group_name === "game_voice" &&
                    "en-gb-claire-phrases" in sprite_packs
                ) {
    ...
    public setSpriteVoiceEnabled(
        sprite_name: ValidSound,
        enabled: boolean,
    ): void {
    ...
    public playStonePlacementSound(
        ...
        try {
            if (
                preferences.get("sound.vibrate-on-stone-placement") &&
                navigator.vibrate
            ) {
