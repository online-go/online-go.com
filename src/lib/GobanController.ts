...
export class GobanController extends EventEmitter<GobanControllerEvents> {
    ...
    public startAutoplay() {
        ...
        const step = () => {
            if (this.goban.mode === "analyze") {
                const before = this.goban.engine.cur_move;
                this.nextMove(undefined, true);

                if (
                    this.goban.engine.cur_move.id === before.id ||
        ...
        /* Enter the playing state before the first synchronous step so that
         * a step that immediately hits the end of navigation can stop
         * autoplay cleanly instead of leaving a "playing" state with no
         * timer scheduled. */
